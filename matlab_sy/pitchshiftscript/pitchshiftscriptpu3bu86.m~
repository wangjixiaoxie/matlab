%THIS SCRIPT IS for r37w41 in 7/06, no pitch shifting just cleaning up.
colordef black
figure
...
aif_outfiles={'386.aif' '386m2.aif' '386p2.aif'}
...
datsong=(1/(normfactor+1))*32768*datsong;
...
for(i=1:length(aif_outfiles))
    ...
    pshift{i}=pshift{i}(st_t*44100:end_t*44100)
end

%this is for shortened volume experiment%
...
for i=1:length(pshift)
    ...
    lng=length(pshift{i});
%instead cut out points if lag is positive
    if(offsetarray(i)>0)
        tester=pshift{i};
...
%now add points at end%
    %if offset is 1, I want to just add the same point)
        ...
        corpshifted{i}=[zeros(-offsetarray(i),1); pshift{i}(1:length(pshift{i})+offsetarray(i))];
        %remove points.
    end  
end       %check to make sure total power is constant
...
for(i=1:length(pshift))
    ...
    plot(fftfreqs(1:88200), log(absarray(1:88200)))
end
...
for i=1:length(corpshiftednormg)
    normvec(1)=max(corpshiftednormg{i});
    normvec(2)=abs(min(corpshiftednormg{i}));
    normfactor=max(normvec);

    corpshiftednormg2{i}=(1/(normfactor))*corpshiftednormg{i};
...
for i=1:length(corpshiftednormg)
    ...
    corpshiftednormg2m{i}=corpshiftednormg2{i}/2;

end
%create soft variants
for i=1:length(corpshiftednormg)
    corpshiftednormg2s{i}=corpshiftednormg2{i}/4

end

%FINALLY, WRITE TO WAV FILES*/
wavstring={'pu3bu86twe2l.wav' 'pu3bu86m10e2l.wav' 'pu3bu86p10e2l.wav' 'pu3bu86reve2l.wav'}
for i=1:length(corpshiftednormg2)
    %corpshiftednormg{i}=[initbuff;corpshiftednormg{i};finalbuff];
    %resample(corpshiftednormg{i},44100,44150);
    wavwrite(corpshiftednormg2{i}, 44100,16,wavstring{i});

end
wavstring2={'pu3bu86twe2m.wav' 'pu3bu86m2e2m.wav' 'pu3bu86p2e2m.wav' 'pu3bu86reve2m.wav'}
for i=1:length(corpshiftednormg2)
    %corpshiftednormg{i}=[initbuff;corpshiftednormg{i};finalbuff];
    %resample(corpshiftednormg{i},44100,44150);
    wavwrite(corpshiftednormg2m{i}, 44100,16,wavstring2{i});

end
wavstring3={'pu3bu86twe2s.wav' 'pu3bu86m10e2s.wav' 'pu3bu86p10e2s.wav' 'pu3bu86reve2s.wav'}
for i=1:length(corpshiftednormg2)
    %corpshiftednormg{i}=[initbuff;corpshiftednormg{i};finalbuff];
    %resample(corpshiftednormg{i},44100,44150);
    wavwrite(corpshiftednormg2s{i}, 44100,16,wavstring3{i});
