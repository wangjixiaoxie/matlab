...
function [ax] = inactivplotre(avls,psin,mx)
...
for ii=1:length(acz)
    ...
    %now get the maxmeans.
    psout(1).pts=[acz(ii) psin.ht(ii) muz(ii) psin.ht(ii)]
    psout(2).pts=[acz(ii) psin.ht(ii) mx  psin.ht(ii)] 
    ...
    if(psin.vec)
        ...
        ln=abs(pts(3)-pts(1));
        if(abs(pts(3))-abs(pts(1)))>0
            col='r'
        else
            col='k'
        end
        plot_arrow(pts(1), pts(2) ,pts(3) ,pts(4),'Linewidth',3,'Color',col,'facecolor', col, 'edgecolor', col,'Headwidth',.1/ln,'Headheight',0.1/ln);
    end
end
