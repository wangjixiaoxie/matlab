...
%randvec=randvec(permout);
    ...
    alt_shift=datnorm_use;
    alt_shift2=datnorm_use;
...
for i=1:length(altvec)
    ...
    alt_shift=alt_shift.*(1.0-mask) + switchsong{1}.*mask;
    alt_shift2=alt_shift2.*(1.0-mask) + switchsong{2}.*mask;
...
imagesr=1000;
altimagevec=0:1/imagesr:totaltime;
imaget=imagevec;
altimagevec(:)=-1;
...
for i=1:length(xlist)-1
    altimagevec(floor(xlist(i)*imagesr):floor(xlist(i+1)*imagesr))=altvec(i);
end

figure;
ax3(1)=subplot(10,2,2:2:16)
[sm,sp,t,f]=evsmooth(alt_shift,fs,0.01);
imagesc(t,f,log(abs(sp)));syn;ylim([0,1e4]);
%plot(0:1/fs:(totaltime-1/fs),dat_shift)
ax3(2)=subplot(10,2,18:2:20)
imagesc(imaget,f,imagevec)
ax3(3)=subplot(10,2,1:2:15)
[sm,sp,t,f]=evsmooth(alt_shift2,fs,0.01);
imagesc(t,f,log(abs(sp)));syn;ylim([0,1e4]);
linkaxes(ax3)
