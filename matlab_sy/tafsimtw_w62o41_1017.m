cd /canary/SY/w62o41/101511_ClickOnly
...
[avnA,t,f]=get_avn('batch.catch','a',0.2,0.2,'','','obs0'); 
...
templaA=mean(avnA(1:2:256,71),2);
size(templaA) %should be 128 point vector  (128 by 1)

%normalize template
templaA(1:6)=0;

templaA=templaA./max(templaA);
%check out template
uievtafsim('batch.catch',templaA);
...
get_trigt2('batch.catch',cntrng,0.4,128,1,1);
...
[vals,trigs]=triglabel('batch.catch','a',1,1,0,1);
...
%pick a TH out of tvals and run triglabel with 30it
cntrng(1).TH=2;
get_trigt('batch.catch',cntrng,0.3,128,1,1);
[vals,trigs]=triglabel('batch.catch','a',1,1,0,1);
...
wrt_templ('w62o41_temp1017.dat',templaA);
