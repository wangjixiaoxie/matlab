...
function [shsall,sumshs, sumbs] = shiftanal4(bs, brdind)
global revstdlimit 
...
for ct=1:length(brdind)
    ...
    [shsout]=calcshsvals(avls,graphvals,bs(bvl),minstd,mineffn);
...
function [shs]=calcshsvals(avls,graphvals,bs,minstd,mineffn)
     wn=avls.wn
...
%      for zz=1 length(allind1)
    for zz=1
        ...
        [shs(jj)]=calcoutvls(flrtmvec, flr_wnon, flr_wnoff, avls, mxshift, outruns, allind1,ii,mineffn);
        end
...
function  [shs]=calcoutvls(tmvec, flron, flroff, avls,mxshift,outruns,allind1,ntind,mineffn)               
