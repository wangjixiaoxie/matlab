...
function [ax] = plot_tmcourse2(sumbs, plotind,axin,STIM,adjx)
        ...
        for ii=1:length(indvl)
            ind=indvl(ii)
                if(~isempty(sumbs.mulist(ind)))
                    ...
                    vls.acz(ii)=sumbs.pct(indvl2);
                    vls.acmean(ii)=sumbs.pct(indvl2);
                    vls.aczerr(ii)=sumbs.acerrz(ntind,indvl2)
                    vls.acstdv(ii)=sumbs.acstdv(ntind,indvl2)/3000;
                    vls.muz(ii)=sumbs.pct(indvl2);
                    vls.mumean(ii)=sumbs.pct(indvl2)
