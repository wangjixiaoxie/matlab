...
function [plotvls,combvls]=plot_consolid1tstv5(phsumbs,sumbs,bs,sts)
figure
...
%         axtmcourse(ii)=subplot(1,bsln,ii);
        plot_tmcourse5(phsumbs(crbs),ps,avls);
%         axis square
        axis([ -2 9 2.25 2.7])
%         ps.plotedge=1;
%         ps.plotedgeybnds=[2 3]
...
crax=subplot(3,2,3:4)
    ...
    ps.plotinitendline= 0
         ...
         ps.plotacline= 1
    plot_tmcourse5(phsumbs(5),ps,avls)
end
...
ps.tmwins{2}=[2 3]
ps.tmwins{3}=[4 5]
ps.axlman=subplot(7,5,1:4);
ps.axmot=subplot(7,5,1:4);
ps.sumax=ps.axlman;
ps.plotacz=0;
ps.interptozero=1;
ps.maxsd=100;
...
figure
ps.axsum=subplot(7,1,5:6)
ps.axz=subplot(717)
ps.plotcv=0;
[plotvls]=plotbarasymp(combvls,ps);
% ps.tmwins=[];
% ps.tmwins{1}=[1 2 3]
% ps.tmwins{2}=[4 5 6]
...
ps.tmwins{2}=[3 4]
ps.tmwins{3}=[5 6]
ps.axlman=subplot(7,5,11:14);
ps.axacz=subplot(7,5,11:14);
ps.axmot=ps.axlman;
ps.plot_type='mop'
ps.runtype='rev'
ps.plotsum=1;
ps.fill=0;
ps.axacz=subplot(7,5,[15]);
ps.sumax=ps.axacz;
...
ps.maxsd=100
% ps.tmwins{3}=[6 7]
ps.axacz=subplot(7,5,16:19);
ps.axlman=subplot(7,5,20);
ps.axmot=subplot(7,5,20);
ps.plotacz=0;
ps.sumax=subplot(7,5,16:19)
ps.fill=0;
...
[phsumdyn,phsumdynasymp,phsumdynrev]=selectdynamicruns4(phsumbs,0,0,0)
[ctinds]=plotcombdynamics7_pharm(phsumdyn,phsumbs,ps)
...
ps.STIM=0;
% figure
[shiftplot,combvls]=plothistoryscatter6(phsumbs,ps)
% figure
ps.axsum=subplot(7,1,1:2)
ps.axz=subplot(717)
ps.plotcv=0;
[plotvls]=plotbarasymp(combvls,ps);
ps.tmwins=[];
...
% 
% ps.axz=subplot(717)
% [plotvls]=plotbarasymp(combvls,ps);axes(ps.axsum)
% axis([-.5 3.5 -1 0])
% box off;
% axes(ps.axz)
% axis([-.5 3.5 -.5 .5])
