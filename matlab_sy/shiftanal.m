...
for ct=1:length(brdind)
  ...
  %to calculate offset
                shft_s(indct).offset=flrtmvec(nwruns)-flr_wnon(end)+1;
                shft_s(indct).offsetall=flrtmvec(runsall)-flr_wnon(1)+1;
  %to calculate this/need to switch into mu coordinates.
                ...
                shft_s(indct).muzall=avls.muz(ntind,runsall);
                shft_s(indct).acerrz=avls.acerracz(ntind,nwruns);
                shft_s(indct).acerrzall=avls.acerracz(ntind,runsall);
                shft_s(indct).muerrz=avls.mustderrz(ntind,nwruns);
                shft_s(indct).muerrzall=avls.mustderrz(ntind,runsall);
