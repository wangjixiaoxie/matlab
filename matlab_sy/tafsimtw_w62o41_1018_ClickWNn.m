cd /canary/SY/w62o41/1018_ClickWN
...
figure
imagesc(log(avnn));syn;

% pull out template slice  if i chose time slice 94
templaN=mean(avnb1(1:2:256,68),2);
size(templaN) %should be 128 point vector  (128 by 1)

%normalize template
templaN(1:6)=0;

templaN=templaN./max(templaN);
%check out template
uievtafsim('batch.catch',templaN);
...
mk_tempf('batch_1018clickWN',templaB,2,'obs0');
...
get_trigt2('batch_1018clickWN',cntrng,0.4,128,1,1); %0.4 = refractory period (sec)
...
[vals,trigs]=triglabel('batch_1018clickWN','b',1,1,0,1);
...
wrt_templ('w62o41_temp1018.dat',templaB);
