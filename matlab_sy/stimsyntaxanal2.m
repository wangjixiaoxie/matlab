...
if (avls.mkfv)
    for indvl=1:length(avls.mkfv)      
        ...
        if(~isempty(avls.pvls{crind}))
            ...
            for jj=1:length(avls.NT)
                ...
                NFFT=avls.NFFT(jj);
                fbins=avls.fbins{jj}
                contourbins=contourbins{jj}
                contourbands=contourbands{jj}
                %%%INSERT
            save BINS_B NFFT fbins tbinshft
            % frequency analysis just for 'b'
            load BINS_B
            NT=avls.NT{crind};avls.PRENT='';PSTNT='';
            fv=findwnote4(bt,NT,PRENT,PSTNT,tbinshft,fbins{crind},4096,1,'obs0');
            pitchdata=jc_pitchcontourFV(fv,1024,1020,1,fbins{crind}(1),fbins{crind}(2),[contourbands ],'obs0');
    ...
    strcmd=['save  ' bt '.mat ' 'fv pitchdata pitchtms'] ;  
    eval(strcmd);
end
