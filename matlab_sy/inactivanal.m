...
for noteind=1:length(chkarray)
    for jj=1:length(avls.analind) 
        ...
        mnvl{noteind}(ssnind)=mean(chkarrayadj{noteind}{ssnind}(:,2))
        stdv{noteind}(ssnind)=std(chkarrayadj{noteind}{ssnind}(:,2))
    
        hstout{noteind}{ssnind}=histc(chkarrayadj{noteind}{ssnind}(:,2),graphvals.edges{noteind})
