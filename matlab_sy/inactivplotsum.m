...
%     downhtcnt=0;
 ...
 for birdind=1:length(bs)      
    ...
    for blocind=1:length(avls.asympind{ntind})
        ...
        subplot(211)
                    scatter(avls.acz(ntind,ind),avls.muz(ntind,ind),10,'k')
    ...
    for blocind=1:length(avls.basind{ntind})
            %set the ind to plot
                ...
                if(~isempty(ps.indtoplot))
                     subplot(211)
                    scatter(avls.acz(ntind,ind),avls.muz(ntind,ind),10,bascolor)
                    hold on;
                    for kk=1:length(ind)
                        test=2
                    end
    ...
    if ~isempty(bs(birdind).contrind)
        ...
        for blocind=1:length(avls.muanal{clind})
            in=avls.muanal{clind}{blocind};
              if(~isempty(in))
                     subplot(211)
                    scatter(avls.acz(clind,in),avls.muz(clind,in),10,concolor)
                    hold on;
                    test=1;
