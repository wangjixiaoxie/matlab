...
%     plotcbin(exsong,ps);
    ...
    exsong(1).path='/oriole7/dir1/r95pk42/screen4'
%     exsong(1).cvl=cvl{2}
    exsong(1).fn='r95pk42_300807_1620.1290.cbin';
    exsong(1).bnds=[2.5 5.1]
    exsong(1).ax=subplot(4,4,1:4)
    ps.detect=0;
    [datout]=plotcbin(exsong,ps);
    hold on;
    ax=gca();
    plotbox([.71 .81 0 10000],'c',ax);
    mxvl=max(abs(datout));
    datoutnrm=datout/(mxvl*1.0001);
    wavwrite(datoutnrm,32000,16,'tst.wav')
    ptsong(1).path=exsong(1).path;
    ptsong(1).fn=exsong(1).fn;
    ptsong(1).bnds=[3.21 3.31]
    ...
    ps.detect=0;
    datout=plotcbin(synsong,ps);
    mxvl=max(abs(datout));
    datoutnrm=datout/(mxvl*1.0001);
    wavwrite(datoutnrm,32000,16,'tst2.wav')
%     exsong(1).path='/oriole/bk48w74/BASELINE4/temptest/files30'
% %     exsong(1).cvl=cvl{2}
%     exsong(1).fn='bk48w74_301009_170051.397.cbin';
