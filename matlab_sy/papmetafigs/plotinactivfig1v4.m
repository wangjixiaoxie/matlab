...
function [shiftplot,combvls,sumdata]=plotinactivfig1v4(sumbs,bs)
figstoplot=[ 2 3 4 5]
...
%         axis square
    ...
    for ii=1:9
        ...
        mucv_vls=crbs.allcv(basruns,ntind);
        
        %find matching acsf runs
        for ii=1:length(basruns)
            acind=find(crbs.mulist==basruns(ii));
            accv_vls(ii)=crbs.allcv(crbs.aclist(acind,1),ntind)
        end
    ...
    sumdata.meancombcv=meancombcv;
     ...
     ind=~isnan(allcombcv.mu)
     
     sumdata.allcvredxn=mean(allcombcv.mu(ind)./allcombcv.ac(ind))
     sumdata.meancvredxn=mean(meancombcv.mu./meancombcv.ac);
    end
