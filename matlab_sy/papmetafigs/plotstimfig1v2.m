    ...
    ps.distht=0.35
     ...
     for ii=1:length(indtoplot)
         ...
         ps.initmean=sumbs(bsnum).initmean
           subplot(6,3,[10+(ii-1)*3:10+(ii-1)*3+1])
           axhist(ii)=gca();
           ps.ax=gca();
           ps.shift_col='k'
           ps.rev_col='r'
...
if (ismember(6,figstoplot))
    ...
    colin{3}=[.6 .6 1]
    MX_TM=4;
    [shiftplot,combvls]=plothistoryscatter1(sumbs,0,0,0,0,axin,axbnds,axscatter,1,colin,MX_TM) 
    axis([-4 4 -1 4])
%     axis square;   
end
if (ismember(7,figstoplot))
    ...
    col{2}{2}=[1 0.7 .7]
      ...
      ps.ax(1)=subplot(6,3 ,15)
      ps.ax(2)=subplot(6,3,18);
      ps.col=col;
      ps.edges=[-4:.5:4]
...
%             end
            hst_tmp=histc(combvls{ii}{jj},edges)
            mnout{ii}{jj}=nanmean(combvls{ii}{jj})
            ster{ii}{jj}=nanstd(combvls{ii}{jj})./sqrt(length(combvls{ii}{jj}));
...
%         end
        ...
        plot([crmn-crster crmn+crster],[0.6 0.6],'Color',col{figcnt}{crvls(hstcount)},'Linewidth',3);
        plot(crmn,0.7,'v', 'Color',col{figcnt}{crvls(hstcount)},'MarkerFaceColor',col{figcnt}{crvls(hstcount)});
        end
