...
function [outstr,combfvst,combstcr,combmatout,avgmod]=plotstimfig2v5(sps,sts,outstr,combstcr,avgmod)
% figstoplot=[ 1 2 3 4 5 6]
% figstoplot=[1 2 3 4 5]
figstoplot=[10];
%settings for first figure (time course)e
if(~exist('outstr'))
    [outstr,combfvst,combstcr]= getresiduals7(sps,sts,1:12);
end
if(~exist('avgmod'))
    [residmod,avgmod,combmatout]=alignresids(outstr,[0 .09])    
end
...
if(ismember(2,figstoplot)) 
   ...
   pitchmeasure=mean(sps(9).OFF_TMS{2})*1000;
   subplot(8,4,[23:24 31:32])
...
if(ismember(3,figstoplot))
    ...
    osind=9;
    binind=3;
    outstrinds=[1 2 3 4 5 6  7 ]
    subplotinds=[5 9 13 17 21 25 29 ]
    ...
    for ii=1:length(outstrinds)
        ...
        axcon(ii)=subplot(8,4,subplotinds(ii))
        ps.ax=gca();
        ps.basind=1
        ps.TIMEBNDS=[.067 .11]
        ps.OFFMEAN=[.079]
...
avgmodind=9
    ...
    ps.frac=1;
    ps.tmtoplot=1:9
    ps.plotinds=1:(length(outstr(crind).tm{1}));
    plotoffsets(outstr(crind),binind,ps)
    axis square;
    box off;
     axis([-.09 .01 -.3 .4])
         plot([-.01 0.07],[0 0],'k--','Linewidth',1);
    end
end
        ...
        acfillcol=[0.6 0.6 0.6]
         ...
         if(ps.plotsigdiff)
             if(isfield(crmod,'initsigtime'))
                for ii=1:length(crmod.initsigtime)
                    init_tm=crmod.initsigtime(ii);
...
%     crdiffind=find(~isempty(crdiff));
    ...
    plot(tmvls(ps.TMTOPLOT),diffvls(ps.TMTOPLOT),'Color','k','Linewidth',2);
    hold on;
    plot([-.08 0],[0 0],'k--');
...
st_tm=onstime(2)/1000+.073-.008-exsong(1).bnds(1);
end_tm=st_tm+256/32000;


%three white lines at 25ms, 35ms, 45ms.
lnx{1}=[st_tm-.025 st_tm-.015]
lny{1}=[2000 2000]

lnx{2}=[st_tm-.035 st_tm-.025]
lny{2}=[2100 2100]

lnx{3}=[st_tm-.045 st_tm-.035]
lny{3}=[2200 2200]
axes(ax2(1))
for ii=1:3
   plot(lnx{ii},lny{ii},'c','Linewidth',3)
   hold on;
