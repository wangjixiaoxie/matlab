...
function [outstr,combfvst,combstcr,combmatout]=plotstimfig2v4(sumbs,sts,sps,outstr,combstcr,combmatout,avgmod)
% figstoplot=[ 1 2 3 4 5 6]
figstoplot=[5 10]
%settings for first figure (time course)
if(~exist('outstr'))
    [outstr,combfvst,combstcr]= getresiduals6(sps,sts,1:length(sps));
    [residmod,avgmod,combmatout]=alignresids(outstr,[.007 .05])    
end
...
%     crdiffind=find(~isempty(crdiff));
    for ii=difflist
        ...
        tmvls=[tmvls outstr.tm{binind}{ii}]
    end
    plot(tmvls,diffvls,'Color',ps.colvec,'Linewidth',2);
    hold on;
