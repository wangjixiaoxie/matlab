...
function [plotvls,stats,ctrlvls]=plotinactivfig1v8(sumbs,bs)
% figstoplot=[ 2 3 4 5]
figstoplot=[ 12] ;
% figstoplot=[9 10]
...
%     subplot(5,2,[2 4]);
    ...
    if(BARPLOT)
    ps.calcz=0;
    [shiftplot,combvls]=plothistoryscatterB(sumbs(1:9),ps) 
    sumdata.MX_TM=ps.MX_TM;   
    combvls=addlidflag(combvls);
    

        figure
        ps.axsum=subplot(6,1,1:4)
        ps.axpct=subplot(6,1,5)
        ps.axbas=subplot(6,1,6)
        ps.TYPE='plotsum'
        ps.calcz=0;
        ps.plotcv=1;
...
%          plotin(5).type=2;
%          plotin(5).nt=2;
%          plotin(5).xvl=[14 15 16]
%          plotin(5).comb=0;
%          plotin(5).drxn=2;
%          plotin(5).norm=0;
%          
         
        [plotvls,stats,ctrlvls]=plotgroupbarinit(combvls,ps,plotin);
        tst=1;
%          ps.calcz=0;
%     [shiftplot,combvls]=plothistoryscatter6(sumbs(1:9),ps) 
%     sumdata.MX_TM=ps.MX_TM;   
%     combvls=addlidflag(combvls);
...
%          ps.marksize=1;
%          ps.splitupdown=1;
%         [plotvls]=plotgroupbar3(combvls,ps);
%         
    ...
    for ii=1:length(indtoplot)
       ...
       avls=getavls(bs,sumbsvl(ii))
        ...
        ps.omitpost=omitpost(ii);
         axraw(ii)=subplot(2,5,ii)
        ptsps.ax=gca();
        ps.ax=gca();
        
        ptsps.plot_triangles=1;
        ptsps.triangle_xvl=13;
        ptsps.SERIAL=0;
        sumdataraw(ii)=inactiv_rawpoints(avls,ptsps,sfact);
%         axis square
        box off
        axhist(ii)=subplot(2,5,ii+5)
        ps.ax=gca(); 
        ps.col{2}='r'
        ps.plot_triangles=1;
        ps.distht=.8;
...
%         axis([0 0.05 0 0.05]);
  ...
  if(ismember(10,figstoplot)) 
    comb.ratvls=((comb.mumean./comb.acmean)-1)*100;
    comb.meanratvls=mean(comb.ratvls);
    comb.sterat=std(comb.ratvls)./sqrt(length(comb.mumean));
   
    combcv.ratvls=((combcv.mumean./combcv.acmean)-1)*100;
    combcv.meanratvls=mean(combcv.ratvls);
    combcv.sterat=std(combcv.ratvls)./sqrt(length(combcv.mumean));
  
   
      
      subplot(221)
   
   bar(0,comb.meanratvls,'FaceColor','none');
   hold on;
   plot([0 0],[comb.meanratvls-comb.sterat comb.meanratvls+comb.sterat],'k');
   
   subplot(223)
   bar(0,combcv.meanratvls,'FaceColor','none');
   hold on;
   plot([0 0],[combcv.meanratvls-combcv.sterat combcv.meanratvls+combcv.sterat],'k');
  end
% sumdata.comb=comb
% sumdata.combcv=combcv;
...
if(ismember(11,figstoplot))    
     ...
     ps.muindvl=2:11
        ...
        subplot(2,3,1:3)
        [os_ph]=plot_tmcourse5MOD2(sumbs(5),ps,avls)
