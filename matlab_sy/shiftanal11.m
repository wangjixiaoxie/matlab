%2.4.10
%calcasympind needs to be modified in order to calculate acsf inds as part
%of asymptote runs.
...
function [sumbs,shsall,sumshs, shsrev] = shiftanal9(bs, brdind)
global revstdlimit 
...
%      for zz=1 length(allind1)
    for zz=1
        ...
        for jj=1:length(wn(ii).on(:,1))
            ...
            [outruns,outrunsall,maxshift(jj),drxn{jj},asympvl{jj}]=findruns(avls,subind,ii,jj,zz,flrtmvec,contrvl,minstd,bs);
            [shs(jj)]=calcoutvls(flrtmvec, flr_wnon, flr_wnoff, avls, maxshift(jj), outruns, outrunsall,allind1,ii,mineffn,drxn{jj},asympvl{jj});
        end
    ...
    function [outruns, outrunsall,mxshift, drxn,asympvl]=findruns(avls,subind,ii,jj,zz,flrtmvec,contrvl,minstd,bs);
        for subindvl=1:3
                ...
                if(mxshift>0)
                 ...
                 ntind=bs.ntind;
                [outruns{4},outrunsall{4},asympvl]=calcasympind(avls,outrunsall{2},ntind)
...
function  [shs]=calcoutvls(tmvec, flron, flroff, avls,mxshift,outruns,outrunsall,allind1,ntind,mineffn,drxn,asympvl,muasympdist,acasympdist)               
  ...
  %to calculate this/need to switch into mu coordinates.
                ...
                if(~isempty(outruns{4}))
                    ...
                    ind=find(avls.tmvc(:,1)<ceil(avls.tmvc(limrun,1)));
