...
function [netdiff] = plot_tmcourse5(crbs, ps,avls)
...
%   
            function [out_tm,out_mn]=mk_acmean(avls,acind)
                ...
                out_tm=nanmean(outtmvls);
                out_mn=nanmean(outptvls);
...
if(ps.plotsep)
    ...
    if(ps.plotallac)
        ...
        for ii=1:length(nozeroind)
            ...
            plot(crtmcomb-start_time,crvlcomb,'k','Linewidth',ps.lw);
            end
if(ps.plotallacpts)
...
%     plot([0 max(endactms)],[vls.basmean,vls.basmean],'k--','Linewidth',2)
    ...
    if(ps.netdiff)
    tmpind=find(avls.mulist(nozeroind)~=0);
