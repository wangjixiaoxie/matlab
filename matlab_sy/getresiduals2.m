...
for spsnum=1:length(ind)
    ...
    for grp=1:length(crsp.datbnds)
        ...
        [outstr(spsnum).outmat{grp}, outstr(spsnum).out_tms{grp},outstr(spsnum).tm{grp},outstr(spsnum).mndiff{grp},outstr(spsnum).mnct{grp},outstr(spsnum).mner{grp},outstr(spsnum).mnresid{grp},outstr(spsnum).steresid{grp},combfvst,combstcr,outstr(spsnum).combctcon{grp},outstr.combfbcon{grp}]=getresid(sts,crsp, grp,mnct, PT_TMS,stbnds,TMBUFF)
    end
end
...
function [residmat,residtms,tm,mndiff,mnctlim,mner,mnresid,steresid,combfvst,combstcr,combctcon,combfbcon] = getresid(sts,crsp,grp,mnct,PT_TMS,stbnds,TMBUFF)
    stind=crsp.stind;
    path=sts(stind).path
    mfile=sts(stind).matfilename;
    ...
    [avls]=load_avls(path,mfile);
    [combfb,combfvst,pitchtms,del_list,TMDIFF,combstcr,combctr]=combinefbcontours(avls, pvlfb);
    [residmat,residtms,combctcon,combfbcon]=mkresiduals(pitchtms,combfb,combfvst,TMBUFF,PT_TMS,mnct,del_list,stbnds,TMDIFF,combctr);
...
function [combfb,combfvst,pitchtms, del_list,TMDIFF,combstcr,combctr]=combinefbcontours(avls,pvlfbind)
    combfvst=[]
    combfb=[];
    combstcr=[];
    combctr=[];
...
function [residmat,mat_tms,combctcon,combfbcon]=mkresiduals(pitchtms,combfb,combfvst,TMBUFF,PT_TMS,mnct, del_list,stbnds,TMDIFF,combctr)            
    combctcon=[];
    combfbcon=[];
    fbind=[];
