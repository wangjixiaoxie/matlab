...
function [os,combfvst,combstcr]= getresiduals5(sps,sts,ind) 
...
for spnum=1:length(ind)
    ...
    for grp=1:length(crsp.datbnds)
        ...
        [os,combfvst,combstcr]=getresid(sts,spnum, crsp,grp,mnct, crsp.RESIDTMS,crsp.OFF_TMS,stbnds,TMBUFF,os)
    end
end
...
function [os,combfvst,combstcr] = getresid(sts,spnum,crsp,grp,mnct,RESID_TMS,OFF_TMS,stbnds,TMBUFF,os)
    ...
    for ii=1:length(OFF_TMS)
        [residmatfb,residmatfboff,residmatct,residmatctoff,residtmsct,residtmsctoff,residtmsfb,residtmsfboff,combctcon,combfbcon]=mkresiduals(pitchtms,combfb,combfvst,TMBUFF,RESID_TMS,OFF_TMS{ii},mnct,del_list,stbnds,TMDIFF,combctr,combct);  
