...
function [shsall,sumshs, sumbs] = shiftanal5(bs, brdind,uselocavls)
global revstdlimit 
...
%         sumbs(bvl).revruns=bs(bvl).revruns
        ...
        sumbs(bvl).revruns{ii}=shsout(ii).revruns;
...
%      for zz=1 length(allind1)
    for zz=1
        ...
        for jj=1:length(wn(ii).on(:,1))
            if jj<=length(bs.wn(ii).revon)
                revon=bs.wn(ii).revon{jj};
            else
                revon=[]
            end
            [flr_wnon, flr_wnoff, subind,flrtmvec]=calcshiftvls(wn, ii, jj,avls,revon);
            for subindvl=1:4
                    if(~isempty(subind{subindvl}))
                        crind=subind{subindvl};%what's going on here?
        ...
        function [flr_wnon, flr_wnoff, subind,flrtmvec]=calcshiftvls(wn,ntin,shin,avls,revon)
                %hack...start by calling the reverse runs, any runs in the
                %four days after wnrevon date
                switchdays=4
            ...
            %subind{1} are all the inds, subind{2} are the asymp inds.
                ...
                %additional analysis to find subind{4}, which are the
                %reverse funs.
                if ~isempty(revon)
                    revon=datenum(revon,'yyyy-mm-dd HH')
                  flrrevon=floor(revon);
                  subind{4}=find((flrtmvec>=flrrevon)&flrtmvec<(flrrevon+4));
                else
                    subind{4}=[];
                end
...
function  [shs]=calcoutvls(tmvec, flron, flroff, avls,mxshift,outruns,allind1,ntind)               
  ...
  %to calculate this/need to switch into mu coordinates.
                ...
                shs.ntype='targ'
                 ...
                 [shs.effvls,shs.combeff,shs.ac_cv, shs.mu_cv]=get_effvls(avls,allind1);
