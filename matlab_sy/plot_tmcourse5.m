  ...
  %this version is rewritten to calculate beginning and endvls
      ...
      vls.zerotime=floor(crbs.tmvc(ps.startind,1))+.5;
        ...
        for ii=1:length(muindvl)
            ...
            vls.rawmuvls{ii}=avls.adjvls{1}{cr_murun}(:,2)/ps.divfac;
...
if(ps.plotsep)
    ...
    if(ps.plotinitendline)
      ...
      if(ps.plotmumarker)
        plot(xvecmu,ps.muht*ones(1,length(xvecmu)),'Marker','<','Color',ps.mucol,'MarkerFaceColor',ps.mucol,'Linestyle','none');  
      end
      if(ps.plotmuline)
      ...
      if(ps.plotrawmu)
         for ii=1:length(vls.rawmuvls)
             ...
             plot(vls.rawmutms{ii}+.6,mean(vls.rawmuvls{ii}),'Marker','<','Color',ps.mucol,'MarkerFaceColor',ps.mucol,'Linestyle','none')
         end
      end
    if(ps.plotallac)
        crvlcomb=[];
        ...
        for ii=1:length(nozeroind)
            ...
            hold on;  
            if(ps.plotacline)
            plot(crtm-start_time,crvl,'o','MarkerFaceColor',ps.accol,'MarkerEdgeColor',ps.accol,'MarkerSize',ps.msize)
...
if(ps.plotallacpts)
            if(avls.aclist(crind,1))
                ...
                hold on;       
                 plot(crvls(:,1)-start_time,crvls(:,2)/ps.divfac,'o','MarkerFaceColor','none','MarkerEdgeColor','k','MarkerSize',2)
                if ps.plotacmarker
                    plot(crvls(:,1)-start_time+.5,mean(crvls(:,2)/ps.divfac),'<','MarkerFaceColor','k','MarkerEdgeColor','none','MarkerSize',3)
                end
                end
