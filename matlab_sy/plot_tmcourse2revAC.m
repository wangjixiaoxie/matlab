...
function [] = plot_tmcourse2revaconly(avls, plotind,ps)
...
%         end
  ...
  function   [vls]=getvals(avls,plotind,ps);
      vls.ntind=ps.ntind;
      ntind=ps.ntind;
      ...
      indvl=plotind
        ...
        for ii=1:length(indvl)
            ind=indvl(ii)
            if(avls.mulist(ind))
                vls.acmean(ii)=avls.acz(ntind,avls.mulist(ind));
                vls.mumean(ii)=avls.muz(ntind,avls.mulist(ind));
                 vls.aczerr(ii)=avls.acerracz(ntind,ind);
                 vls.muzerr(ii)=avls.mustderr(ntind,ind);
            else
...
%             vls.acstdv(ii)=avls.stdv{ntind}(ind)/3000;  
    ...
    vls.rwtms=avls.rawtimes(indvl,1)
...
%     vls.wnoff=sumbs.flr_wnoff
    vls.rwtms=avls.tmvc(avls.aclist(plotind,1),1);
...
if(~exist('sbval'))
  ...
  if(ps.FILL)
        ...
        xvecmu=xvec(nzind);
        xvec=makerow(xvec)
        xvecbas=[min(xvecmu) max(xvecmu)]
        fillx=[xvec xvecmu(end:-1:1)']
        fillx2=[xvecmu' xvecbas(end:-1:1)]
...
% filly2([1 end])=0
    ...
    fill(fillx2,filly2,acfillcol,'edgecolor','w');
%     
    end  
...
if(isfield(ps,'plotlman'))
   if(ps.plotlman)
    ...
    plot(xvec(nzind),muz(nzind),'Color',ps.mucol,'Linewidth',ps.lw);
    plot(xvec(nzind),muz(nzind),'Marker',ps.mumark,'Color',ps.mucol,'Markersize',ps.msize,'MarkerFaceColor',ps.mucol);
...
%     plot([(xvec) ;(xvec)],[(muz-muzer); (muz+muzer)],'Color',ps.mucol,'Linewidth', ps.errlw);
    ...
    hold on;
    plot([0 max(xvec)],[0,0],'k--','Linewidth',2)
