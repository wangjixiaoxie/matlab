...
MAX_NOTES = 10;
% pull out the a-d-d notes and compare them to other d notes
srch_str = 'bc';cont_str='c';
...
while (1)
	...
	if (exist(basename,'file')&exist(fn,'file'))
		...
		for ii = 1:length(s_pos)
			...
			while ((strcmp(labels(ind),cont_str))&(nind<=MAX_NOTES))
				sp_inds=onsets(ind)+[dat_dur(1):dat_dur(2)];
				notes(:,:,nind)=notes(:,:,nind) + ...
				                    abs(sp(:,sp_inds));
			
			        Nnotes(nind) = Nnotes(nind) + 1;
				ind = ind + 1;nind = nind + 1;
				if (ind>length(labels))
					break;
				end
			end
		end
