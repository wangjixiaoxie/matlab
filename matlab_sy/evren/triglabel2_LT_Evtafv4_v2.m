function [vals,trigs]=triglabel2_LT_Evtafv4_v2(batch,NOTE,PRENT,PSTNT,UseDotTrigFile,DoLowerCase,NoDoCatch,ADDX, NoteNum);

%% LT 12/7/14 - v2 =  modified to work with .rec files that are made using EvTAFv4Sim_LT
% i.e. using readrecf_LT_evtafv4
% Reason: it save trigger times for all notes (i.e. evtafv4 can have
% multiple templates (each with multiple columns).  all ttimes are
% combined, and there is a field in the .rec file that lists the note
% indices (e.g. 0, 1, 2...).

% New ARgumetnt:
% NoteNum=0; % then will only look at ttimes for note 0.  Can only do one
% note at a time.

% Arguments:
% NoDoCatch - if 1, then will not skip syls that are catch (I think)
...
while (1)
    ...
    % Only look at the ttimes that are for the note you care about
    TrigsToKeep=find(rd.trignote==NoteNum);
    tt=tt(TrigsToKeep);
    ...
    for ii = 1:length(tt)
        ...
        if (length(ptmp)>0)
            ...
            if ptmp(1)>pre_count && (length(labels)-ptmp(1)+1)>post_count; % index cannot be too close to start of song, or else can't possibly get prenotes
...
% ADD TRIGNOTES (LT)
trigs(cnt).trignote=rd.trignote;
