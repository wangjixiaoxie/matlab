function [fvalsstr]=findwnote3(batchnotmat,NOTE,PRENOTE,POSTNOTE,TIMESHIFT,FVALBND,NFFT,...
    ADDNOTMAT,USEFIT,CHANSPEC,ADDX);
% NEW PINTERP VERSION
% [fvalsstr]=findwnote3(batchnotmat,NOTE,PRENOTE,POSTNOTE,TIMESHIFT,FVALBND,NFFT,...
%                                                       ADDNOTMAT,USEFIT,CHANSPEC,ADDX);
%
%
%fv=findwnote3(bt,NT,PRENT,PSTNT,tbinshft,fbins,NFFT,1,1,'obs0');
%>> fv=findwnote3(bt,NT,PRENT,PSTNT,tbinshft,fbins,NFFT,1,1,'obs0');
    ...
    for ii = 1:length(p)
	    ...
	    fvals=fvals(1:end/2);fdattmp=fdattmp(1:end/2);
            ...
            for kk = 1:size(FVALBND,1)
                ...
                mxtmpvec(kk)=fvals(tmpinds)*fdattmp(tmpinds)./sum(fdattmp(tmpinds));
            end
