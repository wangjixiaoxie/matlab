function [pat]=evgetpatbatch(batchfile,pattern,ADDNOTMAT);
%[pat]=evgetpatbatch(batchfile,pattern,ADDNOTMAT);
...
while (1)
    ...
    fprintf(fout,'%s\n',labels);
    Ndatf = Ndatf + 1;
    datafiles{Ndatf} = filename; %cell arrray of data file names
end
