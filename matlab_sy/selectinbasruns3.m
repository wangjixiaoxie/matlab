...
%MODIFIED to allow return of sumplotrev.

%this option tells whether to plot initind, selected as consecutive mu runs
%over some threshold.

%four output columns
%first field is all the target note runs
%second column is all the control note runs
%third column is all the baseline runs.
%fourth column is summary of all baseline runs.


function [sumplotout,sumplotrevout]=selectinbasruns3(sumbs,init_typ, range, minshift,modstruct)
...
for sumtype=1:2
    ...
    for ii=1:length(sumbs)
         crbs=sumbs(ii)
         if(exist('modstruct'))
            [initind, initday, drxnvec,basind, shftnum]=getshftbasind(crbs, init_typ,range{sumtype},sumtype,ii,modstruct);
         else
              [initind, initday, drxnvec,basind, shftnum]=getshftbasind(crbs, init_typ,range{sumtype},ii,sumtype);
         end
          ...
          targnt=allnote(1);
          [redinitind, indvls,drxnvecnew]=getredinitind(acary, initind,drxnvec,targnt,minshift(sumtype));
             ...
             for jj=1:6
                 ...
                 if(jj<5)
                    ...
                    if(ntind==1)
                        ...
                        if(jj==1)
                            drxnvec_comb=[drxnvec_comb drxnvecnew];
         ...
         [sumplot]=mksumplot(initary,basary,ctrlary, initdaycomb, bsinitnumcomb, initindcomb, bsbasnumcomb, basindcomb,bsctrlnumcomb, bsctrlindcomb, shftnumcomb,drxnvec_comb);
    ...
    function [sumplot]=mksumplot(initary,basary,ctrlary,initdaycomb, bsinitnumcomb, initindcomb, bsbasnumcomb, basindcomb, bsctrlnumcomb,bsctrlindcomb,shftnumcomb,drxnvec_comb)
    ...
    sumplot(1).drxnvec=drxnvec_comb
...
function [initindcomb,initdaycomb, drxnvec_comb,basind, shftnumcomb]=getshftbasind(crbs,init_typ,range,sumtype,bsnum,modstruct);
...
function [sumplot]=calcoff(sumplot);
 ...
 function [outind, threshind,drxnvecnew]=getredinitind(acary, initind,drxnvec,nt,minshift)
