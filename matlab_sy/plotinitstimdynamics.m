...
function [outvlaczcomb,outvlmuzcomb]=plotinitstimdynamics(sumdynin,sumbs,ps)
...
%             
    [acvls]=interp1(smcr.rawtms,aczvl,tm_matrix);
    [muvls]=interp1(smcr.rawtms,muzvl,tm_matrix);
...
%             plot([0:ln-1],outvlaczcomb(ct-1,:),'Color',ps.ac_col,'Linewidth',2)
                tmsind=find(smcr.rawtms<=ps.maxx);
                [out,sortind]=sort(smcr.rawtms(tmsind));
