...
for outnum=1:length(lesind)
    ...
    for indct=1:length(prelesind)
        ...
        if(avls.type{crind}=='PT')
            ...
            outmt(outnum).prelespt(preptct)=ptanal(crind);
            outmt(outnum).pth=crpth;
            preptct=preptct+1;
            outmt(outnum).drxn=avls.drxn{crind}
        else
            outmt(outnum).pth=crpth;
            outmt(outnum).preles_sq(prelesct)=seqanal(crind);
            prelesct=prelesct+1;
        end
    end
    ...
    %do the same for the postlesion experimnents
     for indct=1:length(postlesind)
        ...
        if(avls.type{crind}=='PT')
            ...
            outmt(outnum).pstlespt(pstptct)=ptanal(crind);
            outmt(outnum).drxn=avls.drxn{crind}
            pstptct=pstptct+1;
        else
            
            outmt(outnum).pstles_sq(pstlesct)=seqanal(crind);
    ...
    function [crot]=ptanal(crind)
         ...
         if(isfield(dvl,'SPLITDAY'))
             ...
             for ii=1:length(crbas)
                basvls=[basvls ;vlsout{crbas(ii)}{avls.ANALPTNT(end)}];
             end
             wnvls=vlsout{crwn}{avls.ANALPTNT(end)};
...
%        
       if(SPLIT)
        ...
        for tmvl=1:2
            ...
            trgnum=0;
            for ntvl=1:length(avls.SEQTRGNT{1})
                crnt=avls.NT{avls.SEQTRGNT{1}(ntvl)}
                trgnum=trgnum+length(find(combseqlabels(alltotind)==crnt));
                
            end
               seqout(tmvl)=trgnum/length(alltotind);
