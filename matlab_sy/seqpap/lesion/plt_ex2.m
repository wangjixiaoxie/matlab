...
function [datout]=plt_ex(bs,bsnum)
...
load sumdata.mat;
 ...
 sumdat(2).bas=outdtpst(1).freqbas
 sumdat(2).sh=outdtpst(1).freqsh

%THIS LOOP PLOTS FREQUENCY
for ii=1:2
    subplot(2,2,ii);
    dyind=getdays(dayout,avls.pt_days{ii});
    zerotime=floor(dayout(dyind(1)+avls.pt_start(ii)));
    for crind=dyind
...
for ii=1:2
    subplot(2,2,ii+2);
    dyind=getdays(dayout,avls.seq_days{ii});
    zerotime=floor(dayout(dyind(1)+avls.seq_start(ii)));
    for crind=dyind
