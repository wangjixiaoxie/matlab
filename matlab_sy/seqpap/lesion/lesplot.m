...
for ii=1:length(repeatindspre)
    crind=repeatindspre(ii);
    crst=rpst(crind);
    crdy=crst.MIN
    vls=[crst.pstay(1,crdy) crst.pstay(2,crdy)]
      plot([1 2 ],1-vls,'--','Color',repeatprecolor{ii})
        hold on;
    plot([1 2 ],1-vls,'o','Color',repeatprecolor{ii})
    
end
subplot(235)
for ii=1:length(repeatindspst)
    crind=repeatindspst(ii)
    crst=rpst(crind);
    crdy=crst.MIN
    vls=[crst.pstay(1,crdy) crst.pstay(2,crdy)]
      plot([1 2 ],1-vls,'--','Color',repeatpstcolor{ii})
        hold on;
    plot([1 2 ],1-vls,'o','Color',repeatpstcolor{ii})
