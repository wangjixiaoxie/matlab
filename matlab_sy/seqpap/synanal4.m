...
for ii=1:length(avls.bastms)
   ...
   if(exist('dayout'))
    ...
    dvl.wnind{ii}=find(dayout>=wnbnds(1)&dayout<=wnbnds(2));
    dvl.wn_sl(ii)=find(dayout==(wnbnds(1)+avls.offset_dys(ii)));
    if(~isempty(recbnds)) 
...
if(avls.DOCONT_ANAL)
    if(exist('sumdata.mat'))
        cmd=['save -append  sumdata.mat  tms pout p05out p95out vlsout outnotect dayout avls dvl'];

    eval(cmd);
    else
        cmd=['save sumdata.mat  tms pout p05out p95out vlsout outnotect dayout avls dvl'];
        eval(cmd)
    end
