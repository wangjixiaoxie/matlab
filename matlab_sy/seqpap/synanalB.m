...
for ii=1:length(unq_days)
   ...
   for ntind=1:length(avls.NT)
        ...
        onsout{ii}{ntind}=onsvls{ntind}(ptvlsind);
        offsout{ii}{ntind}=offsvls{ntind}(ptvlsind);
   ...
   res_vec(seqindnt{avls.SEQTRGNT{1}})=1;
   dayout(ii)=crday;
...
if(avls.DOCONTSYNANAL)
    if(exist('sumdata.mat'))
        cmd=['save -append  sumdata.mat  cntseq vlsout outnotect combseqtms onsout offsout combseqlabels dayout avls dvl'];
...
if(avls.DOCONTPTANAL)
    ...
    if(exist('sumdata.mat'))
        cmd=['save -append  sumdata.mat  outnotect dayout combseqtms combseqlabels vlsout onsout offsout avls dvl'];
        eval(cmd); 
    else
        cmd=['save  sumdata.mat  outnotect dayout vlsout onsvls offsvls combseqtms combseqlabels avls '];
        eval(cmd);
    end
end
else
