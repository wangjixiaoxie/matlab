...
function [fvalsstr]=findwnote8(batch,NOTE,PRENOTE,POSTNOTE,...
		        TIMESHIFT,FVALBND,NFFT,USEFIT,CHANSPEC,ADDX,ONSETADJ);
% NEW PINTERP VERSION
...
% %     end
    ...
    for ii = 1:length(p)
        ...
        if(length(onsets)==length(labels))
            ton=onsets(p(ii))-ONSETADJ;toff=offsets(p(ii));
            ...
            trigindtmp=find((rd.ttimes>=ton)&(rd.ttimes<=toff));
                if (length(trigindtmp)>0)
                    if ~isempty(findstr(stimstr,rd.pbname{trigindtmp(1)}))
                        ...
                        STIMTRIG=1;
                        STIMTIME=rd.ttimes(trigindtmp(1))-onsets(p(ii));
                        SOUNDTRIG=0;
                        if(isfield(rd,'catch'))
