...
function [shsall,sumshs, shsrev, sumbs,sumbst] = shiftanal7(bs, brdind)
global revstdlimit 
...
%      for zz=1 length(allind1)
    for zz=1
        ...
        for jj=1:length(wn(ii).on(:,1))
            ...
            [outruns,maxshift(jj),drxn{jj}]=findruns(avls,subind,ii,jj,zz,flrtmvec,contrvl,minstd,bs);
    ...
    function [outruns, mxshift, drxn]=findruns(avls,subind,ii,jj,zz,flrtmvec,contrvl,minstd,bs);
        for subindvl=1:3
                ...
                outruns{4}=calcasympind(avls.acz(ii,:), outruns{2}, mxshift,bs)
...
%   and interpolate through all the gap days                      
 ...
 function [sumbs]=overalleff(sumbs)  
     ...
     function [asympind,asympvl]=calcasympind(aczvls, ind, mxshift,bs)
         ...
         asympvl=mean(aczvls(indcheck(indpass)))
