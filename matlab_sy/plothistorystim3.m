...
%over some threshold.
function [shiftplot,mnout,combvls]=plothistorystim(sumbs,minpts,minshift,norm_asymp)
figure
...
%             muzbaspr_vls{ii,revct}=crbs.muz(basins);
        ...
        analrevshiftinds=find(abs(crbs.acz(revind))>1);
...
%             mnout(bsnm).murev{ii}=mean(crbs.muz(analrevinds));
        ...
        shiftplot(bsnm).acshift{ii}{1}=crbs.acz(analbasinds);
        shiftplot(bsnm).mushift{ii}{1}=crbs.muz(analbasinds);
        shiftplot(bsnm).acshift{ii}{2}=crbs.acz(analshiftinds);
        shiftplot(bsnm).mushift{ii}{2}=crbs.muz(analshiftinds);
        
        shiftplot(bsnm).acshift{ii}{4}=crbs.acz(analrevbasinds);
        shiftplot(bsnm).mushift{ii}{4}=crbs.muz(analrevbasinds);
        shiftplot(bsnm).acshift{ii}{3}=crbs.acz(analrevshiftinds);
        shiftplot(bsnm).mushift{ii}{3}=crbs.muz(analrevshiftinds);
    end
    ...
    plotarrow(shiftplot(bsnm),mnout(bsnm),ax(bsnm),colvec);
   
    axhor(bsnm)=subplot(2,length(sumbs),bsnm+length(sumbs));
    [combvls]=plothorizarrow(shiftplot(bsnm),mnout(bsnm),axhor(bsnm),colvec,combvls);
    
end
linkaxes(ax);

function []=plotarrow(shiftplot,mnout,ax, colvec)
axes(ax);
hold on;
numdrxn=length(shiftplot.drxn);   
for ii=1:numdrxn
    for typevl=1:length(colvec)
...
function [combvls]=plothorizarrow(shiftplot,mnout,ax, colvec,combvls)
axes(ax);
hold on;
numdrxn=length(shiftplot.drxn);   
ct=1;
