%% LT 10/28/14 - changed:
% 1) feature_vect to lt_feature_vect
% 2) line 92 - to has defined as string (the input).
% 3) line 141 - to lt_batchlabel, which uses SegmentNotesJC instead of
% evsegment (since the former is what we actually use in evsonganaly). Also
% entered globals in line 142 (summary: placed globals in the read_training
% subfunction, so it spits out the min_dur, min_int, etc sectioning
% parameters from the training .not.mat files, and uses those to section
% during the testing phase.
% 4) line 437, in "get covariances", replaced all that cov calculation with
%the "cov" function - i tested it to be the same.
% 5) commented out line 510
% all simple things because of errors.
...
if ~exist('id_threshold')|(isnan(id_threshold))|(id_threshold<=0)
    id_threshold=1.5;
end
...
if trainup
    templates = make_templates(templateSylls);
    [template_cov,template_mean] = get_covariances(templates,index);
    strctname = input('What would you like to name this template structure? ' ,'s');
    eval([strctname '.labelIndex = labelIndex;']);
    eval([strctname '.index = index;']);
    eval([strctname '.templates = templates;']);
    eval([strctname '.temp_id = temp_id;']);
    eval([strctname '.template_cov = template_cov;']);
    eval([strctname '.template_mean = template_mean;']);
    sv_name = [strctname '.template_strct.mat'];
    save(char(sv_name), strctname);
end
...
if lblornot
    ...
    while ~isempty(songfile)
        lt_batchlabel(songfile, templates,template_cov,template_mean, index);
        songfile = fscanf(songs_fid,'%s',1);
    end
...
while 1
    ...
    if exist(filtfile)
        ...
        [song,fs]=soundin('', soundfile, filetype); % outputs raw song file
        filtsong=lt_bandpass(song,fs,300,8000); % bandpass filter
        write_filt(filtfile, filtsong, fs); 
    end
    
    load(notefile);
    
    GL_THRESHOLD=threshold;
    GL_MIN_INT=min_int;
    GL_MIN_DUR=min_dur;
    GL_SM_WIN=sm_win;
    REUSENOTMAT=0;
...
for i = 1:size(templateSylls,1)
    templates = [templates; lt_feature_vect(templateSylls{i},32000)];
    if mod(i,100)==0
...
for ii = 1:max(index)
    ...
    %           end
    %      end
    covtemplate=cov(these); % this replaces all of the commented stuff above.
...
%     kdiff(:,6)=kdiff(:,6)+100*(kdiff(:,6)<-50)-100*(kdiff(:,6)>50); LT commented
