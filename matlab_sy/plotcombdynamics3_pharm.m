...
function [ctinds,meanmu,meanac]=plotcombdynamics2_pharm(sumdynin,sumbs,ps)
        ...
        if(ps.plotfixedpoints)
           ...
           for crtmwin=1:length(ps.tmwins)
              ...
              if(~isempty(matchinds))
               meanmu(fixct,crtmwin)=mean(calcmeanstder2(muzvl(matchinds)));
               meanac(fixct,crtmwin)=mean(calcmeanstder2(aczvl(matchinds)));
              else
                  meanmu(fixct,crtmwin)=NaN;
                  meanac(fixct,crtmwin)=NaN;
           ...
           ctinds=[ctinds fixct];
               axes(ps.axmot)
           plotinds=ps.xvls;
           plot([plotinds(notnaind)],[meanmu(fixct,notnaind)],'Color',ps.mu_col,'Marker','o','MarkerSize', 4);
            hold on;
            if(ps.plotlman)
            axes(ps.axlman);
            plot([plotinds(notnaind)],[meanac(fixct,notnaind)],'color','r','Marker','o','MarkerSize', 4);
            end
            hold on;
...
%     end
 ...
 [outmumn]=calcmeanstder2(meanmu(ctinds,:));
 [outacmn]=calcmeanstder2(meanac(ctinds,:));
