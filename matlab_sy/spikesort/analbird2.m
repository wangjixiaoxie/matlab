function [ind]=analbird2(upper,thr,snd,ch,samplerate);
...
%It seems like this program expects that data from a given channel is in
%the same row
% get spike times (stupid method)
for c = 1:1% all channels

  [ind] = findpeaks2(ch(c,:),thr,upper);
  figure(1);
  plot(tvec,ch(c,:));
  hold on;
  spk_tms = tvec(ind);
