function plotrespfn(rsptimes,meanvals,colors,rawsong,numdiv,stimleng,randvecfin,song_fs,stimnames,xlist,linestyles,time_bnds);
...
for plti=1:numdiv
    ...
    if(plti==1)
     ...
     for i=2:length(meanvals(1,:))
            ...
            hold on;
            h(i-1)=plot(mean(rsptimes,2), rat_resp(:,i), 'Linewidth',0.3,'Color',colors(i-1),'Linestyle',linestyles{i-1})
     end
