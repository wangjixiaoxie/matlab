%inactivplot4.m
%this program designed to plot for one specific day. 
%histogram, and arrows.
%input parameters are avls,graphvals,muparam, and histogram,axhandle,notevl
%heightoffset
function [ax] = inactivplot4(avls,graphvals,ps)
runind=[];
if (isempty(ps.ax))
   figure
   ax=gcf;
...
%            arrow([x1(ii),x2(ii)],
%            
%            (x2(ii), y(ii),mark(ii),'Color',col(ii),'MarkerSize',10)
%            
%            plot([x2(ii) x2(ii)],[y(ii)-.3 y(ii)+.3],'Color',col(ii))
        end
