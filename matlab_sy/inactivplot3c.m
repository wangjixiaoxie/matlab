...
function [ax]=inactivplot3c(bs,bsind,rawvl,ppf,ntind)
...
for ii=1:length(indtoplot)
    ...
    ax1(pn)=subplot(ppf,2,pn*2-1)
      ...
      [ps.ax, omin, omax]=inactiv_rawpoints(avls,graphvals,ps);
% 
%                     end
%                     if(jj==1&pn==1)
...
%         hs(2).lw=2;
%         hs(2).ls='--'
%         hs=gethstcomb(hs,mulstind,avls);
...
%                 scale_ln=[0 avls.mnvl{noteval}]
             ...
             axes(ax2(pn))
                box off;
%                 axis([.99 1.02 omin omax])
                plot([.99 1.02], [avls.initmean{ntvl} avls.initmean{ntvl}],'k--')
