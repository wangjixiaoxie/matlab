...
if nargin==1 | isnumeric(loadfile)
    %input_bird_id=input('Input bird number (1) = pu44w54 (2) = g26g23   ');
    ...
    while 1
        ...
        clear peak_labelvec amplitude_at_pitchquant peak_pinterp_labelvec
        clear peak_amp summed_amp  amplitude_16msec_around_pitchquant
        eval(sprintf('load %s.not.mat',fn))
        if length(peak_pinterp_labelvec)~=length(labels)
            error('lengths dont agree')
        end
        if neural_available
%            eval(sprintf('load %s.neuralnot_CH2.mat',fn))
        ...
        if length(Data.channels)>1 & first_ch
            if ~exist('loadfile')
                use_channel=input(['Spike data available for channels ' num2str(Data.channels) '.  use which? ']);
                use_channel_entry_str=num2str(use_channel);
            else
                disp(['Using command line channel number of ' num2str(loadfile)])
                use_channel=loadfile;
            end
            use_channel_id=find(use_channel==Data.channels);
            first_ch=0;
        elseif first_ch
            use_channel=Data.channels;
            use_channel_id=1;
            first_ch=0;
        end
        ...
        %    [neural_tmp, Fs]=soundin('.', fn,['obs' num2str(use_channel)]);
        if Fs==3  | Fs_sound==3;Fs=32000;Fs_sound=32000;end   % I still dont understant this bug
        %
        %         %%%%%%%%%%
        %         % add label 'F' to mark end of file - 1sample long
        %         labels=[labels 'F'];
        %         total_time_tmp=Data.total_n_samples/Data.Fs;
        %         onsets=[onsets; total_time_tmp*1000-1];
        %         offsets=[offsets; total_time_tmp*1000];
        ...
        spiketimes_combo=[spiketimes_combo Data.spiketimes{use_channel}+sum(old_time)];

        total_samples_cat(ct)=Data.total_n_samples;

        SNR=[SNR; Data.SNR(use_channel_id)];


        %        old_time(ct)=Data.total_n_samples/Data.Fs;
        old_time(ct)=Data.total_n_samples/Data.Fs;

        %    old_length=length(neural);
        %    old_time(ct)=length(neural)/Fs;
        %   old_length=length(song_tmp);
        %    old_time(ct)=length(song_tmp)/Fs_sound;
        ct=ct+1;
    end
    ...
    if exist('loadfile')
        eval(sprintf('save combined_data_ch%s labels onsets offsets spiketimes neural L_labels fname_arr Fs  length_song peak_amplitudes summed_amplitudes peak_labelvec peak_pinterp_labelvec SNR amplitude_at_pitchquant  amplitude_16msec_around_pitchquant birdname',num2str(loadfile)))
    else
        save combined_data labels onsets offsets spiketimes neural L_labels fname_arr Fs  length_song peak_amplitudes summed_amplitudes peak_labelvec peak_pinterp_labelvec SNR amplitude_at_pitchquant  amplitude_16msec_around_pitchquant birdname
    end
    if ~neural_available
        return
    end
...
if plot_spect_and_neural_ex
    ...
    while ~sum(true)
        ...
        if sum(true)
            ...
            use_channel_id=find(use_channel==Data.channels);
                    use_channel_entry_str=num2str(use_channel);
                    if exist('use_channel_entry_str')
                        a=get(gca,'title');
                        zzz=get(a,'string');
                        if iscell(zzz)
                            zzz{1}=[zzz{1}  '   Ch. '  use_channel_entry_str];
                            title(zzz,'fontweight','bold')
                        else
                            set(a,'string',[zzz '   Ch. '  use_channel_entry_str])
                        end
                    end
            ...
            if nargin==3        % plots example neural and song
                ...
                disp([ 'Loading sound example'])
                if use_channel<10
                    [song_sample, Fs_song_sample]=evsoundin('.', fn,'obs0');
%                    [song_sample, Fs_song_sample]=soundin('.', fn,'obs0');
                else
                    [song_sample, Fs_song_sample]=evsoundin('.', fn,'obs0r');
%                    [song_sample, Fs_song_sample]=soundin('.', fn,'obs0r');
                end
            ...
            if use_channel==10
                use_channel='1r';
            elseif use_channel==20
                use_channel='2r';
            elseif use_channel==30
                use_channel='3r';
            elseif use_channel==40
                use_channel='4r';
            end
['obs' num2str(use_channel)]
            [neural_sample, Fs_neural_sample]=evsoundin('.', fn,['obs' num2str(use_channel)]);
%            [neural_sample, Fs_neural_sample]=soundin('.', fn,['obs' num2str(use_channel)]);
            Fs_neural_sample=32000;
            if isfield(Data,'lowpass_filter')
                if Data.lowpass_filter(find(use_channel==Data.channels))
...
for x=1:length(id_true)
    ...
    if plot_spect_and_neural_ex & x==1
        ...
        yl=get(gca,'ylim');
        xl=get(gca,'xlim');
        whos ydat        first_spiketrain_unwarped
        plot(first_spiketrain_unwarped,ydat(round(first_spiketrain_unwarped*32000)+1),'ko','markerfacecolor',spike_color,'markersize',5)
        %         for x=1:length(t_points)
        %             plot(first_spiketrain_unwarped(x)*[1 1],yl,'r')
        %         end

        plot(xl,Data.TH(use_channel_id)*[1 1],'b:')
        %plot(xl,Data.TH*[1 1],'b:')
...
whos MAT rate_out vect_x vect_y spiketimes_warped
...
if sort_by_pitch
    ...
    if mark_intervals_and_pitchquant
        for x=1:length(id_true)
            plot(spike_intervals_warped{x},[x x],'ro','markerfacecolor',spike_color,'markersize',2)
        end
    end
end
