...
c    Data=analyze(cbin_name,ch_num,n_clusters,[],0);
end
...
if plot_flag
    ...
    plot([1:Fs]/Fs,dat(id_for_samp),'k');set(gca,'xlim',[0 1]),
...
while ~finished
    ...
    if replot
        ...
        if vert_spike_lims_approved & horiz_spike_lims_approved
            ...
            for z=1:n_clusters
                id_tmp=find(IDX==z);id_tmp=id_tmp(1:min([n_waves_to_show length(id_tmp)]));
...
if plot_flag;title('Ellipses are 95.4% conf intervals (2 S.D.)');xl=get(gca,'xlim');yl=get(gca,'xlim');set(gca,'xlim',xl,'ylim',yl);end
