...
%             vls.acmean(ii)=crbs.acmean(1,cr_murun);
                ...
                vls.rawmutms{ii}=[];
                vls.rawmuvls{ii}=[];
                vls.mumean(ii)=crbs.mumean(cr_murun);
                vls.mutms(ii)=mean(crbs.tmvec(cr_murun,:))'
                vls.acmean(ii)=crbs.acmean(cr_murun);
                vls.actms(ii)=vls.mutms(ii);
                vls.acz(ii)=(vls.acmean(ii)-crbs.initmean)./crbs.initsd
...
%   
            ...
            function []=plotvals(crbs,vls,ps,avls)
...
%     
  ...
  start_time=[crbs.tmvc(ps.startind,1)] ;
...
%     
    hold on;
    plot([0 max(vls.actms)],[vls.basmean,vls.basmean],'k--','Linewidth',2)
