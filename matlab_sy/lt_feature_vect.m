%% LT 10/28/14 - changed line 23 and 92 to use conv, not smooth, since latter was giving me error.
% NOTE to self: this acts on sylalble, which is raw sound file (except a
% bandpass filter, but not smoothed).
function featvect=feature_vect(syllable,Fs)

% featvect=FEATURE_VECT(syllable)
% syllable is raw sound data (usually bandpass filtered)
% Returns a row vector which is the feature vector of a particular syllable.

%% RUN
% ====== Duration of the syllable (ms)
Dur=length(syllable)/Fs;
...
loud=conv(syllable.*syllable,kern')/sum(kern);  % Calculate loudness
...
[sd,sdf]=spectrogram(syllable,hanning(length(syllable)),1,length(syllable),Fs);
...
% ======== IGBNORE - Calculate the max and circular average of the fractional part of log2(f),
...
% =========== IGNORE - Calculate the Cepstrum Peak
