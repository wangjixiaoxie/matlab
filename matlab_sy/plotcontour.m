...
%         plot(pitchtms(ind), mnct(ind)-stect(ind),'k');
        plot(xvls,mnct(ind)/1000,'k','Linewidth',0.5)
            ...
            if(ps.plot_triangles)
                ctmnpt=avls.ctmean(crind);
                fbmnpt=avls.fbmean(crind);
                plot(.11,ctmnpt/3000,'Marker','<','Color','k','MarkerFaceColor','k')
                plot(.11,fbmnpt/3000,'Marker','<','Color','r','MarkerFaceColor','r')
            end
