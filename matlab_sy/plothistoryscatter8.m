...
%for plothistoryscatter8- use the lookup table which assigns basruns for
%each shift.

function [shiftplot,combvls]=plothistoryscatter8(sumbs,ps)
DIFF=0
if(isfield(ps,'DIFF'))
    if(ps.DIFF)
        DIFF=1;
      ...
      if(~ps.DIFF)
       ...
       for typevl=1:5
        for ntvl=ps.NTANAL
           ...
           if(typevl<3)
             adjtms=shiftplot.adjtms{ii}{typevl}{ntvl}
           end
            if(numvls)
