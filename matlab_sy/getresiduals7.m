...
function [os,combfvst,combstcr] = getresid(sts,spnum,crsp,grp,mnct,mnbas,RESID_TMS,OFF_TMS,stbnds,TMBUFF,os)
    ...
    for ii=1:length(OFF_TMS)
        ...
        %calculate 
         initinds=find(pitchtms>OFF_TMS{ii}(1));
         endinds=find(pitchtms<OFF_TMS{ii}(end));
         initind=initinds(1);
         endind=endinds(end);
         overall_ctmn=mean(mean(combct(initind:endind,:)));
         overall_bsmn=mean(mnbas(initind:endind));
         overallctdiff=overall_ctmn-overall_bsmn;
        
        [mndiff,mner,meanoutfrac,mnerfrac,sigvl,pvl]=calcoff(mnbas,pitchtms,residmatfboff,residmatctoff,residtmsfboff,TMDIFF,overallctdiff);
        os(spnum).outmatfb{ii}{grp}=residmatfb;
        os(spnum).outmatct{ii}{grp}=residmatct
        os(spnum).sigvl{ii}{grp}=sigvl;
        os(spnum).pvl{ii}{grp}=pvl;
...
%         os(spnum).mnct{ii}{grp}=mnctlim;
        ...
        os(spnum).tm{ii}=get_tm(crsp.OFF_TMS{ii},crsp.datbnds); 
...
%         os(spnum).stefrac{ii}{grp}=stefrac;
    ...
    %this function calculates the time of offset for short pulse experiment
    function [tmlist] = get_tm(OFF_TMS,datbnds)
        for ii=1:length(datbnds)
            tmlist(ii)=-(mean(OFF_TMS)-mean(datbnds{ii}));
        end
...
%         diff_fbmatbasoff
    ...
    %combfvst(ind).STIMTIME is the actual trigger time for the stim pulse
    [stcr]=getstimtimes(combfvst);
    %del_list is the index of delay times to add to the trigger time
...
function   [meanout,meaner,meanoutfrac,meanerfrac,sigvl,pvl]=calcoff(mnbas,pitchtms,residmatfb,residmatct,residtmsfb,TMDIFF,overallctmn)
...
%             tms=fbtms
        ...
        %looping through all 1404 time inds.
        for ii=1:lnfb
            nzindfb{ii}=find(residmatfb(:,ii)~=0);
            mnfb(ii)=mean(residmatfb(nzindfb{ii},ii));
        end   
        for ii=1:lnct
            nzindct{ii}=find(residmatct(:,ii)~=0);
            mnct(ii)=mean(residmatct(nzindct{ii},ii));
...
%the mean of the average offsets

metmnfb=mean(mnfb);
metmnct=mean(mnct);

[sigvl,pvl]=ttest2(mnfb,mnct);

if(lnfb<lnct)
   
   meanout=metmnfb-metmnct;
   meaner=std(metmnfb-mnct)./sqrt(lnfb);
   meanoutfrac=(metmnfb-metmnct)/metmnct;
   resids=(metmnfb-mnct)./metmnct;
   meanerfrac=std(resids)./sqrt(lnfb);
   
else
    meanout=metmnfb-metmnct;
    meaner=std(mnfb-metmnct)./sqrt(lnct);
    meanoutfrac=(metmnfb-metmnct)/metmnct;
    resids=(mnfb-metmnct)./metmnct;
    meanerfrac=std(resids)./sqrt(lnct);
end
...
%                 
%                 stefrac(ii)=std((residfb-residct)./residct)./sqrt(minln);
%                 
%             end
