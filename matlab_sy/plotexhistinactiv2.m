                ...
                lnvls{2}=length(crmuvls);
                 ...
                 if(isfield(avls,'crfbindlim'))
                    crmuvls=avls.ptvls{crind}(avls.crfbindlim{crind},2);
                     ...
                     crmuvls=avls.ptvls{crind}(avls.crfbind{crind},2);
                 end
...
%             end
            numvls=[1 2]
            for ii=numvls
                stairs(credges/3000,crvls{ii},'Color',ps.col{ii},'Linewidth',1)
                hold on;
            %    plot([mnbas+stdbas mnbas+stdbas], [0 1], 'c--')
            %    plot([mnbas-stdbas mnbas-stdbas], [0 1], 'c--')
                if(ii==1)
            plot([ps.initmean(ps.ntind)/3000 ps.initmean(ps.ntind)/3000],[0 1],'k--','Linewidth',1)
                end    
            text(credges(end-3)/3000,max(crvls{2})-(ii/10),['n=' num2str(lnvls{ii})],'Color',ps.col{ii});
                if(ps.plot_triangles)
                    plot(crmn{ii}/3000,dht,'Marker','v','MarkerEdgeColor','none','MarkerFaceColor',ps.col{ii},'MarkerSize',5)
...
%                 end
            ...
            if(isfield(ps,'premean'))
                    plot(ps.premean/3000,dht,'Marker','v','MarkerEdgeColor','none','MarkerFaceColor','k','MarkerSize',5)
