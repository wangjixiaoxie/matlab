...
function [sumplotout,sumplotrevout]=selectinbasruns4(sumbs,init_typ, range, minshift,min_notes,modstruct)
    ...
    for bsnum=1:length(sumbs)
         ...
         if(exist('modstruct'))
            [initind, initday, drxnvec,basind, shftnum,asympvl]=getshftbasind(crbs, init_typ,range{sumtype},sumtype,bsnum,modstruct,min_notes);
              ...
              [initind, initday, drxnvec,basind, shftnum,asympvl]=getshftbasind(crbs, init_typ,range{sumtype},sumtype,bsnum, min_notes);
         end
          ...
          datst{2}.drxnvec=drxnvecnew;
              ...
              for runvl=1:3
                ...
                %looping through the four categories to fill
                 [datout]= setvals(datout,nt, runvl, crbs,datst,bsnum,FLAG)
...
%                 sumplot(runvl).contreff=datout(runvl).contreff;
                ...
                sumplot(runvl).FLAG=datout(runvl).flagvls
...
function [initindcomb,initdaycomb, drxnvec_comb,basind, shftnumcomb,asympcomb]=getshftbasind(crbs,init_typ,range,sumtype,bsnum,min_notes,modstruct);
   ...
   if (init_typ==1)
       ...
       for jj=1:length(inruns)
           if(~isempty(inruns{jj}))
                ...
                if(indrxn{jj}=='up')
                        drxnvec=ones(length(modind),1);
                    else
                    drxnvec=zeros(length(modind),1);
                end
...
function [sumplot]=calcoff(sumplot);
 ...
 function [outind, redinds,drxnvecnew,asympvlnew]=getredinitind2(crbs, initind,drxnvec,asympvl,minshift)
    ...
    asympvlnew=asympvl(redinds);
     ...
     %FUNCTION SETVALS
     function [datout]= setvals(datout,nt, runvl, crbs,datst,bsnum,FLAG)
                datvls=datst{runvl}.ind;
                    bsvls=bsnum*ones(length(crbs.acz(nt, datvls)),1);
                    flagvls=FLAG*ones(length(crbs.acz(nt,datvls)),1);
%                 if(~isempty(nt))
                    ...
                    datout(runvl).flagvls=[datout(runvl).flagvls; flagvls];
...
%                 else
%                     datout(runvl).bsnm=[datout(runvl).bsnum;bsvls]
%                     
%                 end 
