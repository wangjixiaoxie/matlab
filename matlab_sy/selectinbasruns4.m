...
function [sumplotout,sumplotrevout]=selectinbasruns4(sumbs,init_typ, range, minshift,modstruct)
...
% for sumtype=1:2
    ...
    for bsnum=1:length(sumbs)
         crbs=sumbs(bsnum)
         if(exist('modstruct'))
            [initind, initday, drxnvec,basind, shftnum,asympvl]=getshftbasind(crbs, init_typ,range{sumtype},sumtype,bsnum,modstruct);
         else
              [initind, initday, drxnvec,basind, shftnum,asympvl]=getshftbasind(crbs, init_typ,range{sumtype},sumtype,bsnum);
         end
...
function [initindcomb,initdaycomb, drxnvec_comb,basind, shftnumcomb,asympcomb]=getshftbasind(crbs,init_typ,range,sumtype,bsnum,modstruct);
   asympcomb=[];
   initindcomb=[];
   initdaycomb=[];
   shftnumcomb=[];
   ...
   if (init_typ==1)
       ...
       for jj=1:length(inruns)
           if(~isempty(inruns{jj}))
                ...
                asympvl=crbs.asympvl{jj}*ones(length(initind),1);
                    if(indrxn{jj}=='up')
