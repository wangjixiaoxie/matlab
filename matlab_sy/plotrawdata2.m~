%for use with sam's singlechannel data.
...
cbinfile='pu15pu64_030407_1417.665.cbin'

rawwav='/doyale4/twarren/pu15pu64/initscreen/stimfiles2/1564vl4n.wav';
if(~exist('rawsong'))
...
cmd=['load ' cbinfile '.neuralnot_CH1.mat']
eval(cmd);
...
% end
figure(h);
for ii=1:1%length(stimf(:,1))
    scfctor(1)=1.2-.2*(ii-1);
    scfctor(2)=1.2-.2*(ii);
    
    hght=sfct*min(data)
   
            drawyrasters(spkts, hght,COLORS(ii));
            hold on;
