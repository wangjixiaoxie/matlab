...
%         else
%           adjtms=smcr.adjtmts(shiftind);
%           %purpose of this is so that there's only one time per day.
%           [adjtms,aczvl,muzvl]=adj_vls(adjtms,aczvl,muzvl);
%         end
         
if(~isempty(adjtms))
 if((max(adjtms)>=ps.minx)&&(min(adjtms)<=ps.minx))
...
function [yout]=norm_onevls(yin);
 ...
 function[adjtms,aczvl,muzvl]=adj_vls(adjtms,aczvl,muzvl);