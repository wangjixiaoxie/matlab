...
function [sumbs] = metstimanal3(bs, brdind)
global revstdlimit 
...
%         sumbs(bvl).acasympdist{ii}=shsout(ii).acasympdist;
    ...
    sumbs(bvl).tmvc=sumbs(bvl).tmvec;
...
%      for zz=1 length(allind1)
    for zz=1
        ...
        for jj=1:length(wn(ii).on(:,1))
            ...
            [outruns,maxshift(jj),drxn{jj},asympvl{jj}]=findruns(avls,subind,ii,jj,zz,flrtmvec,contrvl,minstd,bs);
        ...
        if(~isempty(wnrev))
            if(~isempty(wnrev(ii).on))
                for kk=1:length(wnrev(ii).on(:,1))
                    ...
                    [revruns,drxn{kk},asympruns{kk},asympvl{kk}]=findrevruns(avls,subind,ii,flrtmvec,bs);
                    [shsrev(kk)]=calcoutrevvls(flrtmvec,flr_wnon,flr_wnoff,avls,revruns,allind1,ii,drxn{kk},asympruns{kk},asympvl{kk});
                end
            else
                shsrev=[];
            end
    ...
    function [outruns, mxshift, drxn,asympvl]=findruns(avls,subind,ii,jj,zz,flrtmvec,contrvl,minstd,bs);
...
%                     runs=crind(matchind);
                ...
                if(mxshift>0)
                 ...
                 %calculate outruns{4}, asympind.
                    ...
                    [outruns{4},asympvl]=calcasympind(avls,avls.acz(ii,:),outruns{2})
                
                test=1
        
        function [revruns,drxn,asympruns,asympvl]=findrevruns(avls,subind,ii,flrtmvec,bs)
%                     
                avls.acz=avls.catchz
                avls.muz=avls.stimz
...
%                 
  ...
  function  [shsrv]=calcoutrevvls(tmvec, flron, flroff, avls,outruns,allind1,ntind,drxn,asympruns,asympvl)               
                shsrv.drxn=drxn;
