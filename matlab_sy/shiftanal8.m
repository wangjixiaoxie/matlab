...
function [shsall,sumshs, shsrev, sumbs] = shiftanal8(bs, brdind)
global revstdlimit 
...
%      for zz=1 length(allind1)
    for zz=1
        ...
        for jj=1:length(wn(ii).on(:,1))
            ...
            [shs(jj)]=calcoutvls(flrtmvec, flr_wnon, flr_wnoff, avls, maxshift(jj), outruns, allind1,ii,mineffn,drxn{jj},asympvl{jj},muasympdist,acasympdist);
        end
...
function  [shs]=calcoutvls(tmvec, flron, flroff, avls,mxshift,outruns,allind1,ntind,mineffn,drxn,asympvl,muasympdist,acasympdist)               
