...
function [pitch_data,TimeAxis]=jc_pitchmat1024sk(shifted,N,OVERLAP,sigma,F_low,F_high,harms,filetype,returnAV,PlotSpec)

% shifted - each row is the raw data for one rendition of the syllable of
% interest - note that you lose the first and last 1024 points
% shited could have many rows (i.e. many renditions of syllable)
% N - default is 1024
...
% returnAV should be 0

% JC says the result would be most reliable when F_low and F_high are around the
% harmonic band with highest power and harms is 1.
...
%sonodvA program   for i=1:size(pitch_data,1);avg(i)=mean(pitch_data(i,:));end
...
% SK added the following part.
TimeAxis = ((0:timebins-1).*((N-OVERLAP)/SAMPLING)) + 1024/SAMPLING/2;

if PlotSpec == 1;
    figure
    specgram(shifted,1024,SAMPLING,512,500); hold on
    colormap(flipud(gray)); 
    caxis([-30 30])
    plot(TimeAxis, pitch_data,'r-'); hold on
    ylim([F_low,F_high])
