function PitchContour_AutoCorr
    ...
    for n=1:length(Batch(:,1))
        ...
        if length(NotePosition)>0
            ...
            elseif strcmp(ext, '.cbin')
                [wavfile,Freq] = ReadCbinFile(wavfilename);
                wavfile = wavfile(:,1);
                wavfile = wavfile / max(wavfile);
            end
            flow = 250;
            nfir = 512;
            ndelay = fix(nfir/2) ;
            ...
            for m=1:length(NotePosition)
                ...
                Note = [Silence;Note;Silence]; % add 5 ms of silent periods before and after the not/net/song14/data/skojima/SongAnalysis2/Xlesion_adults/Selected_Song/org25ppl30/post8wk/undir4FFe
                ...
                if K==1;
                    figure
                    nfft = 1024; Fs=44100; window = 512; noverlap = 500;
%                    specgram(Note, nfft, Fs, window, noverlap); hold on % for wave file w/ sampling freq=44100                
                    specgram(Note, nfft, Freq, window, noverlap); hold on
                    colormap jet
...
%                     ylim([0 7000])
                ...
                PlotSpec = 1;
                if Freq ~= 32000
                    error('Sooyoon: sampling frequency is expected to be 32000 for obs0 (cbin file). Check file type');
                end
...
disp('Calculating autocorrelation functions...')
saveas(gcf,['PitchContour_Overlay_',NoteType,'.fig'])
...
for n=1:length(ContourSegChange(:,1))
    %disp(['Calculating autocorrelation function... (',num2str(n),'/',num2str(length(ContourSegChange(:,1))),')']);
