...
for ifn=1:length(ff)
    ...
    p=findstr(labels,[PRENOTE,NOTE,POSTNOTE])+length(PRENOTE);% note indices
...
%             changed triginftmp to encompass all notes, not just
%             triggered.
            trigindtmp=find(p==p(ii));
           
            if (length(trigindtmp)>0)
                TRIG=1;
                if (isfield(rd,'catch'))
%                     ISCATCH=rd.catch(trigindtmp); I commented out
%                 else
                    ISCATCH=-1;
                end
...
%virtual trigger
	    ...
	    fdattmp=fdattmp(1:end/2);
            ...
            if(trigindtmp)    
                trigtm=onsets(trigindtmp)*1e-3; % was rd.ttimes.
