function [SeqDepPitch_AcrossBirds, PARAMS] = lt_seq_dep_pitch_ACROSSBIRDS_CORR_and_PCA(SeqDepPitch_AcrossBirds, PARAMS)
...
for i=1:NumBirds;
    ...
    for ii=1:numexperiments;
        ...
        elseif isempty(tmp2) && isempty(tmp1);
            ...
            saveON=1;
            [Params, AllDays_RegExpr] = lt_seq_dep_pitch_RegExpr(Params, AllDays_RawDatStruct, saveON);
...
%% =============== PLOT ALL SYLS/ALL EXPTS, in one plot
% in PCA space
% N_dimensions=input('How many PCA dimensions to use? '); 
% NOTE ---> THIS HAS NOT BEEN IMPLEMENTED YET!!!! (currently takes all
% dimensions, so is the same as z-score space)
...
for i=1:NumBirds;
    ...
    for ii=1:num_experiments;
       ...
       plotcol=[rand rand rand];
        ...
        pcmean_ALL=[pcmean_ALL; [pcmean(1) pcmean(2)]];
        pcSD_all=[pcSD_all; [pcSD(1) pcSD(2)]];
