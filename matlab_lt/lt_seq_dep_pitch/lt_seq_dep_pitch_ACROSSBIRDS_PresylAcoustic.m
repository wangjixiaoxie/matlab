...
%% DO presyl tend to have higher acoustic similaerity?
...
Yacoustic{1}=SeqDepPitch_AcrossBirds.AllSyllables.AcousticDist_all(inds2);


% ------------ Presyl diff
inds2=SeqDepPitch_AcrossBirds.AllSyllables.PreSimilar_all==0 & inds1;

Yacoustic{2}=SeqDepPitch_AcrossBirds.AllSyllables.AcousticDist_all(inds2);

% -------- PLOT
lt_plot_distributionPlot(Yacoustic);

distributionPlot(Yacoustic,'histOpt',0, 'showMM', 6);

p=ranksum(Yacoustic{1}, Yacoustic{2});
lt_plot_pvalue(p, 'ranksum');


%% ============================== SCATTER PLOT OF LEARNING VS ACOUSTIC (for presyl similar and diff)
[fignums_alreadyused, hfigs, count]=lt_plot_MultSubplotsFigs(SubplotsPerFig, subplotrows, subplotcols, fignums_alreadyused, hfigs, count);
title('Similar(BK=PRESIM)');
...
Y_acoust=SeqDepPitch_AcrossBirds.AllSyllables.AcousticDist_all(inds2);
Y_learn=SeqDepPitch_AcrossBirds.AllSyllables.LearningRelTarg_all(inds2);

[~,~,~,~,~,~, hplot]=lt_regress(Y_learn, Y_acoust, 1, 0, 0);
set(hplot, 'Color', 'k');
