function [SeqDepPitch_AcrossBirds, PARAMS] = lt_seq_dep_pitch_ACROSSBIRDS_PLOT(SeqDepPitch_AcrossBirds, PARAMS)
...
write_syl=1; % if 1, then writes syl name (with target, bird,e tc)
...
for i=1:NumBirds;
    ...
    for ii=1:numexperiments;
        ...
        for j=1:length(syllist);
            ...
            % - Learning
            try
                learning=SeqDepPitch_AcrossBirds.birds{i}.experiment{ii}.Syl_ID_Dimensions.(syl).LEARNING.consolid_start_rel_targ;
            catch err
                learning = nan;
                SeqDepPitch_AcrossBirds.birds{i}.experiment{ii}.Syl_ID_Dimensions.(syl).LEARNING.consolid_start_rel_targ=nan;
            end
            ...
            % ==== plot
            if is_similar==1;
                lt_plot(eucl_dist, learning, {'Color','b'});
            else
                lt_plot(eucl_dist, learning, {'Color','r'});
            end
        end
    end
end
