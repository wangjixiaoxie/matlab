function lt_context_PLOT_sub3(SORTED_DATA, ng_Ind1, ng_Ind2, plotcol, PhaseNum)
...
%     for i=1:length(X);
%         lt_plot(X(i), mean(Y2{i})-mean(Y1{i}), {'Color', plotcol});
%         
%         Yall=[Yall; mean(Y2{i})-mean(Y1{i})];
%     end
...
%     xlim([0 3]);
    ...
    Ymean=mean(Yall);
    Ysem=lt_sem(Yall);
    lt_plot_bar(X(end)+1, Ymean, {'Color','k' , 'Errors', Ysem, 'Color',plotcol})
    end
