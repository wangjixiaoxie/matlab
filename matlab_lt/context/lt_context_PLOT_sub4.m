function [Yvals]=lt_context_PLOT_sub4(SORTED_DATA, ng_Ind1, ng_Ind2, plotcol, PhaseNum, xval)
% Yvals; vector of difference scores that are plotted
...
%     
    ...
    Yall=[];   
    for i=1:length(X);
        lt_plot(xval, median(Y2{i})-median(Y1{i}), {'Color', plotcol});
        
        Yall=[Yall; median(Y2{i})-median(Y1{i})];
        
%         lt_plot(X(i), median(Y2{i})-median(Y1{i}), {'Color', plotcol});
%         
%         Yall=[Yall; median(Y2{i})-median(Y1{i})];
%         
...
%     xlim([0 3]);
    ...
    Ymean=mean(Yall);
    Ysem=lt_sem(Yall);
    lt_plot_bar(xval+0.1, Ymean, {'Color','k' , 'Errors', Ysem, 'Color',plotcol})
    end
