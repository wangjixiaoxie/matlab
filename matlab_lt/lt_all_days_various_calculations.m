...
%     findwnote2tw_yes_or_no=input('perform findwnote2tw (to get things like lables, onsets, offsets, etc) ? ', 's');
    ...
    get_transitions_yes_or_no=input('perform lt_get_all_transition... on each day? ','s');
    lt_db_check_yes_or_no=input('perform lt_db_check_..._BIRDTAF to get hit rates, frequencies, for pitch training? ','s');
