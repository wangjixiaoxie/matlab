...
first_day = input('first day to analysis? ','s');
...
if strcmp(transitions_lt_question,'y')==1;
    ...
    all_days_all_analysis=lt_save_data_over_experiment_to_structure_TRANSITIONS(batch, first_day, all_days_all_analysis);
...
if strcmp(seq_func_day_question,'y')==1;
    ...
    for i=1:i_max; % for each metric
        for j=1:length(syllables_seq_func);
            ...
            all_days_all_analysis=lt_save_data_over_experiment_to_structure_SEQFUNC(batch,syllables_seq_func(j),metrics{i},0,first_day,all_days_all_analysis);
    ...
    if strcmp(multiple_pitch.parameters.days{1},first_day)==1;
        ...
        for i=1:length(syllables_pitch);
            for j=1:number_of_days;
                all_days_all_analysis.data{j}.all_songs.(syllables_pitch(i)).pitch.FF=...
                    multiple_pitch.FF.(syllables_pitch(i)).time_and_FF{j};
                all_days_all_analysis.data{j}.summary_of_day.(syllables_pitch(i)).pitch.mean=multiple_pitch.FF.(syllables_pitch(i)).mean_FF{j};
                all_days_all_analysis.data{j}.summary_of_day.(syllables_pitch(i)).pitch.sd = multiple_pitch.FF.(syllables_pitch(i)).sd_FF{j};
            end
...
if strcmp(syl_rendition_question,'y')==1;
    ...
    for i=1:length(all_days_various.syl_rendition_amount.syllables); % use set of syllables specific to this analysis, as there are syls for which I rendition amnt is calculated, but structure analysis was not.
        if strcmp(all_days_various.parameters.days{1},first_day)==1;
            ...
            for j=1:number_of_days;
                all_days_all_analysis.data{j}.summary_of_day.(all_days_various.syl_rendition_amount.syllables(i)).syl_rendition_amount= ...
    ...
    if strcmp(all_days_various.parameters.days{1},first_day)==1;
        ...
        for j=1:number_of_days;
            for ii=1:length(trig_syl);
                all_days_all_analysis.data{j}.WN_hit_rate.per_song.(trig_syl(ii)).hits_labels_allhits=all_days_various.trig_label.vals.(trig_syl(ii)){j};
                all_days_all_analysis.data{j}.WN_hit_rate.per_song.(trig_syl(ii)).all_values=all_days_various.trig_label.trigs.(trig_syl(ii)){j};
                % compile over day
                all_days_all_analysis.data{j}.WN_hit_rate.all_songs.(trig_syl(ii)).hits_labels_allhits=sum(all_days_various.trig_label.vals.(trig_syl(ii)){j});
                % get stats (hit rate, false positive over hits)
                try
                    all_days_all_analysis.data{j}.WN_hit_rate.all_songs.(trig_syl(ii)).hits_divide_labels=all_days_all_analysis.data{j}.WN_hit_rate.all_songs.(trig_syl(ii)).hits_labels_allhits(1)/all_days_all_analysis.data{j}.WN_hit_rate.all_songs.(trig_syl(ii)).hits_labels_allhits(2); %hit/labels
                    all_days_all_analysis.data{j}.WN_hit_rate.all_songs.(trig_syl(ii)).FalsePos=all_days_all_analysis.data{j}.WN_hit_rate.all_songs.(trig_syl(ii)).hits_labels_allhits(3)-all_days_all_analysis.data{j}.WN_hit_rate.all_songs.(trig_syl(ii)).hits_labels_allhits(1); % amnt of f.p.
                    all_days_all_analysis.data{j}.WN_hit_rate.all_songs.(trig_syl(ii)).FalsePos_divide_TotalWN=all_days_all_analysis.data{j}.WN_hit_rate.all_songs.(trig_syl(ii)).FalsePos/all_days_all_analysis.data{j}.WN_hit_rate.all_songs.(trig_syl(ii)).hits_labels_allhits(3);  % f.p./AllWN
                catch err
                end
            end
        end
...
save(['all_days_all_analysis_' timestamp '_' phrase things_analyzed{1:end}],'all_days_all_analysis')
