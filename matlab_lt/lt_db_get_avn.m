function [avsp, t, f, filename, avsm]=db_get_avn(bt,NT,PRETIME,POSTTIME,PRENT,POSTNT,CS,PLTIT);
...
for ii=1:length(files)
	...
	if (length(pp)>0)
		...
		for jj=1:length(pp)
			...
			if(spcnt==0)
				avsp{ii}{jj}(:,:)=abs(sp); % LT: changed from having j as 3rd dim in matrix. now I can have diff. sized specs.
                filename{ii}{jj} = fn;
				avsm{ii}{jj}=sm; %
				spcnt=1;
			else
