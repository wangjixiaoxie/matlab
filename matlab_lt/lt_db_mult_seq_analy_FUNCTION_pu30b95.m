% LT 1/23/14: "_pu30b95" means that I made path as an argument input to use
% with birds that are not mine (i.e. in diff. path directory).

% LT modified 9/30 to change save directory.
% also modifiel line 75 to get time difference between occurances of the transitions (i.e.
% offset of first to onset of second) in order to ask whether the magnitude
% of history dependence depends on how long ago the last transition accured.

%To get a quick calculation of transition probabilites with more than 2
%branch points.

% LT added FUNCTION 1/3/14 - made script a function (for ease of use with
% lt_all_day_various... which iterates over all days and runs this function
% each day. 

function lt_db_mult_seq_analy_FUNCTION(path,bird_name,date_of_exp,phrase,batchfile,num_tp,tpofinterest,con_or_div)
...
% which_computer=2;
% multiple_trans.parameters.computer = ['/bluejay' num2str(which_computer) '/lucas/birds'];
...
% end
    multiple_trans.parameters.computer = path;

% clear which_computer

multiple_trans.parameters.bird_name=bird_name;
% multiple_trans.parameters.bird_name = input('What is the name of your bird? ', 's');
multiple_trans.parameters.date_of_exp=date_of_exp;
...
multiple_trans.parameters.phrase=phrase;
...
multiple_trans.parameters.batchfile=batchfile;
...
multiple_trans.parameters.num_tp=num_tp;
...
% end
multiple_trans.parameters.tpofinterest=tpofinterest;
...
%     multiple_trans.parameters.con_or_div = input('Convergence or divergence? (con or div)  ','s');
        multiple_trans.parameters.con_or_div = con_or_div;
...
[multiple_trans.onsets multiple_trans.offsets]=lt_get_onsets_and_offsets(multiple_trans.parameters.batchfile);
...
%         % here calculates the time interval each occurrence of a transition (any
%         % of the desired transitions)
%         indices_of_last_syl_in_transition{i}=strfind(multiple_trans.syl_order{i}, multiple_trans.parameters.tpofinterest{j})+multiple_trans.parameters.tp_length{j};
%         indices_of_first_syl_in_transition{i}=strfind(multiple_trans.syl_order{i}, multiple_trans.parameters.tpofinterest{j});
%         multiple_trans.onsets_of_first_syl_in_transition{i}=multiple_trans.onsets{i}(indices_of_first_syl_in_transition{i});
%         multiple_trans.onsets_of_first_syl_in_transition{i}=multiple_trans.onsets_of_first_syl_in_transition{i}(2:end); % trhow out first one - duration before this one is undefined
%         multiple_trans.offsets_of_last_syl_in_transition{i}=multiple_trans.offsets{i}(indices_of_last_syl_in_transition{i});
%         multiple_trans.offsets_of_last_syl_in_transition{i}=multiple_trans.offsets_of_last_syl_in_transition{i}(1:end-1); % throw out last, as there is no defined interval after
%         
%         multiple_trans.duration_between_transitions{i}=multiple_trans.onsets_of_first_syl_in_transition{i}-multiple_trans.offsets_of_last_syl_in_transition{i};
    end
    temp_syl_order = sort(temp_syl_order);
    % here calculates the time interval each occurrence of a transition (any
    % of the desired transitions) (LT 9/2013)
    if strcmpi(multiple_trans.parameters.con_or_div, 'div')
    indices_of_last_syl_in_transition{i}=temp_syl_order;
    indices_of_first_syl_in_transition{i}=temp_syl_order-1;

    elseif strcmpi(multiple_trans.parameters.con_or_div, 'con')
    indices_of_last_syl_in_transition{i}=temp_syl_order+1;
    indices_of_first_syl_in_transition{i}=temp_syl_order;
    end
    ...
    multiple_trans.duration_between_transitions.separated_by_song{i}=multiple_trans.onsets_of_first_syl_in_transition{i}-multiple_trans.offsets_of_last_syl_in_transition{i};
...
multiple_trans.duration_between_transitions.separated_by_song{i}; %final array for inter-transition durations)
multiple_trans.syl_order = multiple_trans.syl_order(~cellfun('isempty',multiple_trans.syl_order));

%% Do transitions temporally closer together have higher history dependence?  (LT 9/2013)

% FIRST, plot histogram of interval times and ask how you want to bin
% them.

% compiling all times, to make a histogram
multiple_trans.duration_between_transitions.durations_compiled=[];
for i=1:length(multiple_trans.duration_between_transitions.separated_by_song);
    multiple_trans.duration_between_transitions.durations_compiled=[multiple_trans.duration_between_transitions.durations_compiled;...
        multiple_trans.duration_between_transitions.separated_by_song{i}];
end


hist(multiple_trans.duration_between_transitions.durations_compiled,200)

% PAUSED HERE, IT IS DIFFICULT TO DO THIS BECAUSE THE INTERVALS BETWEEN
% DIFFERENT TRANSITIONS ARE DIFFERENT ANYWAY.
...
for j = 1:length(multiple_trans.parameters.tpofinterest)
    if length(multiple_trans.trans_prob.day.trans_prob_per_song(:,j))<=5;
        multiple_trans.parameters.running_average_window=length(multiple_trans.trans_prob.day.trans_prob_per_song(:,j))-1;
