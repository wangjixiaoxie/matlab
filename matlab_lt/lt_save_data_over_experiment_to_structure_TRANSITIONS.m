function [all_days_all_analysis] = lt_save_data_over_experiment_to_structure(batch, first_day,structure)
...
while ischar(tline)
    ...
    all_days_all_analysis.data{rel_day}.all_songs.all_trans=all_trans;
    
    
                    if isfield(all_days_all_analysis.data{rel_day},'date')==0;
                    all_days_all_analysis.data{rel_day}.date=all_trans.parameters.date_of_exp;
                else
                    if strcmp(all_days_all_analysis.data{rel_day}.date,all_trans.parameters.date_of_exp)==0;
                        disp(['all_trans error: the (index-wise) same all_days_all_analysis structure day was ' all_days_all_analysis.data{rel_day}.date...
