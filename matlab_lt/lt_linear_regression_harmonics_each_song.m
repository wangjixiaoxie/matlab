% perform linear regression of the ratio of 2nd to first harmonic versus
...
disp('manually move to /home/lucas/data/song/pu13bk43/all_days_harmonics_summary/lt_plot_harmonics_structure... that contains your time data');
disp('then load the structure and type return')

% NOTE: LO2 TRANSFORM TAKEN OF HARMONICS RATIOS.
...
transition_times=find(time_diff>0.0028); % 10 seconds in units of hours. transition times are times for 1st note.
...
[r_regression(i), slope_regression(i), offset_regression(i)]=regression(1:length(song_indices{i}),log2(all_harmonics_second_over_first{index}(song_indices{i})))
...
% % plot histogram of pre vs. during WN.
% plot(
...
mkdir(['lt_linear_regression_harmonics_' num2str(suffix2(1)) num2str(suffix2(2)) num2str(suffix2(3)) '_' num2str(suffix2(4)) num2str(suffix2(5))]);
cd(['lt_linear_regression_harmonics_' num2str(suffix2(1)) num2str(suffix2(2)) num2str(suffix2(3)) '_' num2str(suffix2(4)) num2str(suffix2(5))])
