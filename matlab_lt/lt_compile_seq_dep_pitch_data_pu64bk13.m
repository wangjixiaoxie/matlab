...
%% pu64 specific: harms = 2, not 1; sigma = 1.5, not 1
...
% pitch contour
pc_harms=2; % harmonics to take weighted avg over. 1 or 2 is good.
plotON=1;
...
for i=1:NumSyls;
    try % i.e. maybe this day does not have this syllable
        fvalsstr_forpc_all.(syllables{i}) =...
            findwnote2tw_v4_LT(batch, syllables{i},'',-0.006,...
...
for i=1:NumSyls;
    [PC_raw.(syllables{i}), pc_F{i}, pc_T{i}, ~]=jc_pitchcontourFV_LT(fvalsstr_forpc_all.(syllables{i}),...
        1024,1020,1.5, frequency_range{i}(1),...
