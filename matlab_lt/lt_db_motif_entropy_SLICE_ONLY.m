...
for j = 1:length(motif)
    ...
    if isempty(absolute_time.(motif(j))) == 1
        ...
        for k = 1:size(spec,3)
            ...
            if j==2; % here tell it to take slice 69 for the b syllable.
                [wiener_entropy] = db_calc_wiener_entropy( spec(:,69,k) );
                syl_we.(motif(j))(k) = wiener_entropy;
            elseif j==1; % do normal thing for 'a' syllable
                [wiener_entropy] = db_calc_wiener_entropy( spec(:,:,k) );
                wiener_entropy = wiener_entropy(time >= 0 & time <= syl_duration.(motif(j))(k)); %keeps the wiener entropy values only when the syllable was sung
