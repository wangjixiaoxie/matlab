...
plotAllClustCombined=0; % if 0, then does not plot combined clusters
...
if plotAllClustCombined==1
    tmptmp=1; 
else
    tmptmp=0; % only plot each cluster (not combining clusters)
end

for k=1:numclusters+tmptmp
...
hsplots=[];
    ...
    for i=1:numsegs
        ...
        if isempty(inds)
            ...
            for iii=1:length(inds)
                ...
                line([spktime spktime], [i-0.4 i+0.4], 'Color', plotcols{k}, 'LineWidth',1);
            end
        end
    ...
    lt_subtitle(['batch: ' Params.batchf '; chan: ' Params.channel_board '; clust=' num2str(k)]);
linkaxes(hsplots, 'x');
