...
function lt_neural_AlgnWavclus(batchf, channel_board)
...
tt=[1:length(AllNeural_old)]/frequency_parameters.amplifier_sample_rate;
...
%     
    % - d. neural (cat)
    [fignums_alreadyused, hfigs, figcount, hsplot]=lt_plot_MultSubplotsFigs('', subplotrows, subplotcols, fignums_alreadyused, hfigs, figcount);
    [datfilt] =lt_neural_filter(neural_cat.data(indsamp), frequency_parameters);
    plot(tt(indsamp)-timefirst, datfilt, 'k');
    hsplots=[hsplots hsplot];
    title('neural, concated file (extracted spikes)');
    
    % overlay spike times (cat)
    numclust=size(spikes_cat.cluster_class,2);
    plotcols={'m', 'r','c', 'b'};
    for i=1:numclust
...
numchunks=4; % to divide spikes by time
binsize=numtotspk/numchunks;
hsplots2=[];
for k=1:numchunks
    hsplot2=lt_subplot(5, numchunks, (5-1)*numchunks+k); hold on;
