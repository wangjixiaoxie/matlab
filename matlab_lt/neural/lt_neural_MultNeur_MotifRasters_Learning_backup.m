function lt_neural_MultNeur_MotifRasters_Learning(NeuronDatabase, motif_regexpr_str, motif_predur, ...
    motif_postdur, LinScaleGlobal, FFparams, WNchangeDateStrings, OnlyPlotNoHit)
%% 
...
doclose=input('type "c" to close figures', 's');
if strcmp(doclose, 'c');
    close all;
end
% 
...
for i=1:NumNeurons
    ...
    for m=1:NumMotifs
        ...
        for j=1:(numtrials-TrialBinSize+1);
            ...
            [x, ~, ~, ymean_hz, ~, ~, ystd_hz] = lt_neural_plotRastMean(Yspks(trialbins), window, windshift, 0, '');
