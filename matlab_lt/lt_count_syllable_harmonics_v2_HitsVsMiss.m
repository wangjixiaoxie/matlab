...
for i = 1:length(make_temp.parameters.syllables)
    ...
    make_temp.parameters.slices{i} =input(['What are the slice coordinates for ' make_temp.parameters.syllables(i) '?\n(i.e. 60 or 70:82)  ']);
    for ind=1:size(make_temp.all_spectrograms{i},3)
        ...
        % here normalize each slice so that area under entire curve =1.
        area_under_curve=trapz(temporary_slice);
        temporary_slice_1=temporary_slice./area_under_curve;
        make_temp.all_spectrogram_slices_AreaNormalized{i}(:,ind)=temporary_slice_1;
        area_normalization_check(ind)=trapz(temporary_slice_1); % check that these values are all 1.
        
        
        % here normalize each slice so that max peak = 1.
        temporary_slice_2=temporary_slice./max(temporary_slice);
        make_temp.all_spectrogram_slices{i}(:,ind)=temporary_slice_2;
        
        % here not normalized to anything.
        temporary_slice_3=temporary_slice;
        make_temp.all_spectrogram_slices_NotNormalized{i}(:,ind)=temporary_slice_3;
        
        clear temporary_slice
        
        
    end
    % plot the area normalizd and not normalized versions
    
    
    make_temp.template{i} = mean(make_temp.all_spectrogram_slices{i},2);

    
    make_temp.template_AreaNormalized{i} = mean(make_temp.all_spectrogram_slices_AreaNormalized{i},2);
    
    make_temp.template_NotNormalized{i} = mean(make_temp.all_spectrogram_slices_NotNormalized{i},2);
    
    figure(2); subplot(2,2,2*i-1); hold on; plot(make_temp.template_AreaNormalized{i}); title(['Area Normalized - syllable ' make_temp.parameters.syllables(i)]); xlim([0 128])
    figure(2); subplot(2,2,2*i); hold on; plot(make_temp.template_NotNormalized{i}); title(['Not normalized - syllable ' make_temp.parameters.syllables(i)]); xlim([0 128])
    ...
    %area normalized
       figure(4); subplot(2,2,2*i-1); hold on;  plot(make_temp.all_spectrogram_slices_AreaNormalized{i}), title(['Area Normalized. syllable ' make_temp.parameters.syllables(i) ' a single instance.'])
    %         saveas(figure(gcf), [save_dir '/all_spectrogram_slices_' make_temp.parameters.syllables(i)],'fig');
    xlim([0 120])
    ...
    % not normalized
        %area normalized
       figure(4); subplot(2,2,2*i); hold on;  plot(make_temp.all_spectrogram_slices_NotNormalized{i}), title(['Not Normalized. syllable ' make_temp.parameters.syllables(i) ' a single instance.'])
    %         saveas(figure(gcf), [save_dir '/all_spectrogram_slices_' make_temp.parameters.syllables(i)],'fig');
    xlim([0 120])
    ...
    % get average # and STD of peaks
    make_temp.mean_number_of_peaks{i}=mean(make_temp.all_spectrogram_peakamounts{i});
    make_temp.STD_number_of_peaks{i}=std(make_temp.all_spectrogram_peakamounts{i});
    ...
    for kk=1:3; %kk = harmonic #
        for ind=1:size(make_temp.all_spectrograms{i},3);
            ...
            make_temp.all_spectrogram_ampl_harmonics_AreaNormalized{i}(kk,ind)=max(make_temp.all_spectrogram_slices_AreaNormalized{i}(harm(kk,1):harm(kk,2),ind));
            make_temp.all_spectrogram_ampl_harmonics_NotNormalized{i}(kk,ind)=max(make_temp.all_spectrogram_slices_NotNormalized{i}(harm(kk,1):harm(kk,2),ind));
        end
    ...
    %mean and std amplitude for each harmonic
        ...
        % area nomalized
        make_temp.all_relative_harmonic_ampl_means_AreaNormalized{i}(kk)=mean(make_temp.all_spectrogram_ampl_harmonics_AreaNormalized{i}(kk,:));
        make_temp.all_relative_harmonic_ampl_STD_AreaNormalized{i}(kk)=std(make_temp.all_spectrogram_ampl_harmonics_AreaNormalized{i}(kk,:));
        
        % not normalized
        make_temp.all_relative_harmonic_ampl_means_NotNormalized{i}(kk)=mean(make_temp.all_spectrogram_ampl_harmonics_NotNormalized{i}(kk,:));
        make_temp.all_relative_harmonic_ampl_STD_NotNormalized{i}(kk)=std(make_temp.all_spectrogram_ampl_harmonics_NotNormalized{i}(kk,:));
    ...
    make_temp.all_spectrogram_spectral_means_AreaNormalized{i}=nan(1,size(make_temp.all_spectrograms{i},3));
    make_temp.all_spectrogram_spectral_means_NotNormalized{i}=nan(1,size(make_temp.all_spectrograms{i},3));
    ...
    for ind=1:size(make_temp.all_spectrograms{i},3);
        ...
        make_temp.all_spectrogram_spectral_means_AreaNormalized{i}(ind)=(sum((make_temp.all_spectrogram_ampl_harmonics_AreaNormalized{i}(:,ind).*[1;2;3]),1))./(sum(make_temp.all_spectrogram_ampl_harmonics_AreaNormalized{i}(:,ind),1));
        make_temp.all_spectrogram_spectral_means_NotNormalized{i}(ind)=(sum((make_temp.all_spectrogram_ampl_harmonics_NotNormalized{i}(:,ind).*[1;2;3]),1))./(sum(make_temp.all_spectrogram_ampl_harmonics_NotNormalized{i}(:,ind),1));
    end
    ...
    make_temp.mean_over_renditions_spectral_mean_AreaNormalized{i}=mean(make_temp.all_spectrogram_spectral_means_AreaNormalized{i}(:));
    make_temp.STD_over_renditions_spectral_mean_AreaNormalized{i}=std(make_temp.all_spectrogram_spectral_means_AreaNormalized{i}(:))

    make_temp.mean_over_renditions_spectral_mean_NotNormalized{i}=mean(make_temp.all_spectrogram_spectral_means_NotNormalized{i}(:));
    make_temp.STD_over_renditions_spectral_mean_NotNormalized{i}=std(make_temp.all_spectrogram_spectral_means_NotNormalized{i}(:))
    ...
    figure(3); subplot(2,2,2*i-1); hold on
    errorbar([1 2 3], make_temp.all_relative_harmonic_ampl_means_AreaNormalized{i},make_temp.all_relative_harmonic_ampl_STD_AreaNormalized{i});xlim([0.9 3.1])
    line4=line([make_temp.mean_over_renditions_spectral_mean_AreaNormalized{i} make_temp.mean_over_renditions_spectral_mean_AreaNormalized{i}],[0.01 max(make_temp.all_relative_harmonic_ampl_means_AreaNormalized{i})+0.05]);
    line5=line([make_temp.mean_over_renditions_spectral_mean_AreaNormalized{i}+make_temp.STD_over_renditions_spectral_mean_AreaNormalized{i} make_temp.mean_over_renditions_spectral_mean_AreaNormalized{i}+make_temp.STD_over_renditions_spectral_mean_AreaNormalized{i}],[0.01 max(make_temp.all_relative_harmonic_ampl_means_AreaNormalized{i})+0.05]), set(line5,'LineStyle','--');
    line6=line([make_temp.mean_over_renditions_spectral_mean_AreaNormalized{i}-make_temp.STD_over_renditions_spectral_mean_AreaNormalized{i} make_temp.mean_over_renditions_spectral_mean_AreaNormalized{i}-make_temp.STD_over_renditions_spectral_mean_AreaNormalized{i}],[0.01 max(make_temp.all_relative_harmonic_ampl_means_AreaNormalized{i})+0.05]), set(line6,'LineStyle','--');
    title({['Area-Normalized. mean amplitude of harmonics. syllable ' make_temp.parameters.syllables(i)],['vertical lines - spectral mean +/ STD']})
    
    figure(3); subplot(2,2,2*i); hold on
    errorbar([1 2 3], make_temp.all_relative_harmonic_ampl_means_NotNormalized{i},make_temp.all_relative_harmonic_ampl_STD_NotNormalized{i});xlim([0.9 3.1])
    line7=line([make_temp.mean_over_renditions_spectral_mean_NotNormalized{i} make_temp.mean_over_renditions_spectral_mean_NotNormalized{i}],[0.01 max(make_temp.all_relative_harmonic_ampl_means_NotNormalized{i})]);
    line8=line([make_temp.mean_over_renditions_spectral_mean_NotNormalized{i}+make_temp.STD_over_renditions_spectral_mean_NotNormalized{i} make_temp.mean_over_renditions_spectral_mean_NotNormalized{i}+make_temp.STD_over_renditions_spectral_mean_NotNormalized{i}],[0.01 max(make_temp.all_relative_harmonic_ampl_means_NotNormalized{i})]), set(line8,'LineStyle','--');
    line9=line([make_temp.mean_over_renditions_spectral_mean_NotNormalized{i}-make_temp.STD_over_renditions_spectral_mean_NotNormalized{i} make_temp.mean_over_renditions_spectral_mean_NotNormalized{i}-make_temp.STD_over_renditions_spectral_mean_NotNormalized{i}],[0.01 max(make_temp.all_relative_harmonic_ampl_means_NotNormalized{i})]), set(line9,'LineStyle','--');
    title({['Not-Normalized. mean amplitude of harmonics. syllable ' make_temp.parameters.syllables(i)],['vertical lines - spectral mean +/ STD']})
    ...
    line10=line([make_temp.mean_number_of_peaks{i} make_temp.mean_number_of_peaks{i}],[0.01 20]);
