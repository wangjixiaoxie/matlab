...
function [days, gap_durations_over_day]=lt_all_days_get_gap_durations(transitions, date_and_time_of_various_struct, bird_folder);
...
num_days=size(all_days_various.findwnote2tw.fvalsstr,2);

for jj=1:num_days; % days
    if size(all_days_various.findwnote2tw.fvalsstr{jj},2)>0;
    for ii=1:size(all_days_various.findwnote2tw.fvalsstr{jj},2) % number of songs
        % GET DATA PER SONG
%         labels{jj,ii}=all_days_various.findwnote2tw.fvalsstr{jj}(ii).lbl;
%         onsets{jj,ii}=all_days_various.findwnote2tw.fvalsstr{jj}(ii).ons;
%         offsets{jj,ii}=all_days_various.findwnote2tw.fvalsstr{jj}(ii).offs;
        labels{jj}{ii}=all_days_various.findwnote2tw.fvalsstr{jj}(ii).lbl;
        onsets{jj}{ii}=all_days_various.findwnote2tw.fvalsstr{jj}(ii).ons;
        offsets{jj}{ii}=all_days_various.findwnote2tw.fvalsstr{jj}(ii).offs;
    end
...
for tr=1:size(transitions,2);
    for jj=1:num_days;
        if size(all_days_various.findwnote2tw.fvalsstr{jj},2)>0; % continue if day contains songs
            for ii=1:size(all_days_various.findwnote2tw.fvalsstr{jj},2) % iterate over songs
