function [StatsStruct, Params]=lt_Opto_Stim_analy_PLOT_TimeWindow(StatsStruct,Params)
...
%% REMOVE OUTLIERS FROM TIME WINDOWS
...
% end
% 
% subtitle('Statistics in temporal windows');
% 
...
legend(hfig5,Params.FieldsToCheck);
subtitle('Pitch (MEAN), all trials, sorted by time window and trial');
...
legend(hfig5,Params.FieldsToCheck);
subtitle('Pitch (MEDIAN), all trials, sorted by time window and trial');
...
subtitle('Amplitude, all trials, sorted by time window and trial');
...
subtitle('Entropy, all trials, sorted by time window and trial');
...
legend(hfig5,Params.FieldsToCheck);
subtitle('Pitch (MEAN/SEM), sorted by time window and trial');
...
legend(hfig5,Params.FieldsToCheck);
subtitle('Pitch (MEDIAN/SEM), sorted by time window and trial');
...
legend(hfig6,Params.FieldsToCheck);
subtitle('Amplitude, sorted by time window and trial');
...
legend(hfig7,Params.FieldsToCheck);
subtitle('Entropy, sorted by time window and trial');
...
Params.PLOT_TimeWindow_savedir=[Params.PLOT_savedir '/TimeWindow_' tstamp];

mkdir(Params.PLOT_TimeWindow_savedir);
cd(Params.PLOT_TimeWindow_savedir);
