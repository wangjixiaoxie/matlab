...
function [pitch_data,F,T,spec]=jc_pitchcontourFV(fvals,N,OVERLAP,sigma,F_low,F_high,harms,filetype)
%% LT 11/12/14 - notes
% will take a sliding median and see whether that is good at removing rapid
% transients in the contours - added to end, and added output. DID NOT DO.

% ADDED spec as output (spectrogram).
...
% Modified to output F and T (time and freq bins for the spectrogram)
...
for found_note=1:NumberOfNotes
    [S,F,T]=spectrogram(shifted(found_note,:),w,OVERLAP,N,SAMPLING);
    ...
    for currenttime_bin=1:size(sonogram,2)
        ...
        for i=1:length(harms)
            ...
            % lt added, use findpeaks, becuase it will avoid choosing a
            % bin at the boundaries, which can lead to noisy contours.
            % BUT REMOVED, AS FINDPEAKS IS VERY SLOW!!!
%             if Ind==1 || Ind==length(freq_window) % edges, don't want that.
%                 [Pow,Ind]=findpeaks(freq_window,'sortstr','descend');
%                 if numel(Ind)==0; % means that probably in noisy part of syllable, so all we have to work with is edge;
%                     [Pow,Ind]=max(freq_window); % should get largest edge
%                 end
%                 Ind=Ind(1);
%                 Pow=Pow(1);
%             end
    ...
    spec{found_note}=sonogram; % lt added
end
