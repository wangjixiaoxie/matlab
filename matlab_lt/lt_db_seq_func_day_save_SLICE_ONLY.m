% LT MODIFIED 8/13/13 to take a single slice (same as used to make templates, slice 69 for syllable b, where onset is 60), 
% when calcualting entropy, similar to spectral entropy calculation done in
% that Brainard paper looking at RA and variability of song.
...
for k = 1:length(data_type)
    for j = 1:max(size(motif))
        ...
        elseif ~isempty(strfind(data_type{k},'ent')) == 1;
            ...
            [data time] = lt_db_motif_entropy_SLICE_ONLY(batchfile,motif{j});
            
            %saves entropy data
            save(['/' computer '/all_days_entropy_' phrase '/' birdname '_' date '_entropy_' motif{j} '_SliceOnlyForB_WholeSyllForA.mat'],...
                'data', 'time')
