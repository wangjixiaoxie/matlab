...
%% modified by LT 9/26/13 

% this is the correct version to use with lt_db_mult_seq_analy
...
if strcmpi(input(['Is this the path for computer: ' '/bluejay' num2str(which_computer) '/lucas/birds/  '],'s'),'y') == 1
    parameters.computer = ['/bluejay' num2str(which_computer) '/lucas/birds/'];
else
...
if strcmpi(input('is this a WN learning experiment? ','s'),'y')==1;
    ...
    parameters.day_begin = input('How many days of baseline?  ');
    parameters.day_duration = input('How many days of post-manipulation data?  ');
end

%% Loads list of .mat files

% parameters.directory_list = dir([parameters.computer parameters.birdname '/all_days_sequence_' parameters.phrase '/' '*' parameters.con_or_div '*']);
% 
...
% 
% 
% ls([parameters.computer parameters.birdname '/all_days_sequence_' parameters.phrase])
% parameters.date_of_mat_file_creation=input('what date did you create the .mat file? they are listed in the display ','s');
% 
% % fid = fopen([parameters.computer parameters.birdname '/all_days_sequence_' parameters.phrase '/' parameters.birdname '_' parameters.date_of_mat_file_creation '_' parameters.con_or_div '.mat'],'r');
% 
% fid = fopen([parameters.computer parameters.birdname '/all_days_sequence_' parameters.phrase '/' parameters.birdname '_' parameters.date_of_mat_file_creation '_' parameters.con_or_div],'r');
...
% I REWROTE TO THE THE FOLLOWING:

parameters.directory_list = dir([parameters.computer parameters.birdname '/all_days_sequence_' parameters.phrase '/' '*' parameters.con_or_div '*']);

%list the dates of the experiment and puts them in chronological order
for i = 1:length(parameters.directory_list)
    parameters.dates(i,:) = parameters.directory_list(i).name(length(parameters.birdname)+2:length(parameters.birdname)+10);
end
parameters.dates = sort(datenum(parameters.dates));
parameters.dates = datestr(parameters.dates, 'ddmmmyyyy');

for i=1:length(parameters.directory_list);
%     parameters.file_names(i,:) = parameters.directory_list(i).name; don't
%     use, leads to plotted dates not corresponding to analysis dates.
    parameters.file_names(i,:) = [parameters.birdname '_' parameters.dates(i,:) '_' parameters.con_or_div '.mat'];
end
