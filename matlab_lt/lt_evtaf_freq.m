...
for ii=1:length(ff)
	...
	for jj=1:length(tt) %I should remove this part?  only uses syllables that were hit in catch trials.  I want all syllables that I labele.d  
		pp=find((onsets<=tt(jj)*1e3)&(offsets>=tt(jj)*1e3)); %find syllables that are within trigger.  
