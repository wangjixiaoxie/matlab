...
%% LT 1/29/15 - Modified for Marc Schmidt lab - does not need naming and organization of directories the way I do it.
% Everything I did is noted btw lines 91 to 105.  Now the code asks the user to enter things.
% Lines 113 and 134 have also been modified so that it gets the correct directory.
...
%     varargin{2}= cntrng_values (format: {[min max threshold] "and" "y"
%     "y"}, e.g. cntrng_values{1}={[5 14 1] 'or' 'n' 'y'}, one cell entry
%     for each column
...
% Added to code:
parameters.data_date=input('What is the date of the data? (e.g. 28Jul2014) ','s');
parameters.phrase = input('What is an identifying phrase of this experiment (for IDing purposes)? (e.g. HVCstim)', 's');

% to get one dir up:
curr_dir=pwd;
cd ..
parameters.computer=pwd;
cd(curr_dir);
% ---------------------------------------------------------
...
if get_offline_match==1; % then need template info
    ...
    load([parameters.computer '/' template_name '.dat'])
...
% Save dir
SaveDir1=[parameters.computer '/check_hit_templ_freq/'];
