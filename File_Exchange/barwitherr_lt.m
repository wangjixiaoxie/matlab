%% LT modified 10/10 - line 84 - values should be y values, which is varargin{2}, not varargin{1}(which is x value_.
...
if nargin < 3
    ...
    if isscalar(varargin{2}) || ischar(varargin{2})
        ...
        values = varargin{2}; % new (LT)
%         values = varargin{1}; OLD (LT)
        xOrder = 1:size(values,1);
    else
