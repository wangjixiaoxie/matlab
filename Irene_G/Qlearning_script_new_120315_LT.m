...
elseif (nargin >= 4) && (N < 10000)
    ...
    for n=1:N
     ...
     % if from state s action a is impossible, try another action
        if (s_new==S) && (p==0);
            while (s_new==S) && (p==0);
            A=[min(A):max(A)];% transform A into array
            A=A(A~=a); %pick an action that wasn't the one from before
            a = randi([1,max(A)]);
            s_new =0;
                 while ((p < p_s_new) && (s_new < S)) % try if it is possible
                 s_new = s_new+1;
                 p = p + P(s,s_new,a); %if not, go to the top and try another action.
                 end                   %For each state there are possible actions
            end;                       %so it should move on eventually when it finds a possible one
        end 
            
         
       % when possible (s,s_new,a) pair is found, calucate reward 
       r = R(s,s_new,a); 
