...
smoothed(1)=smoothed(1)/3;
smoothed(length(smoothed))=smoothed(length(smoothed))/3;
...
for i=1:(length(mins)-1)		%these are initials and
	...
	if max(smoothed(mins(i):mins(length(mins)-f)))-smoothed(mins(i))<ex*mean(smoothed)|max(smoothed(mins(i):mins(length(mins)-f)))-smoothed(mins(length(mins)-f))<ex*mean(smoothed)
