...
while 1,
  ...
  elseif strncmp(line, 'Samples =', 9),
     nscans = sscanf(line, 'Samples = %d');
	  if isempty(nscans), %Evren used ~isint, isint doesn't seem to exist, had to change strategy.
        error(['Attempt to read Sample # in recfile ' recfile ' failed.']);
	  end
  elseif strncmp(line, 'Chans =', 7),
	  nchans = sscanf(line, 'Chans = %d');
	  if isempty(nchans),   %Evren used ~isint, isint doesn't seem to exist, had to change strategy.
