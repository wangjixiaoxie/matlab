...
%% load /cardinal/FiguresA/Alldata216.mat
            ...
            scaling=(mean(actual(:,281)));
            scalingP=(mean(predicted(:,281)));
...
%             ciplot(mean(predicted)/scalingP-(std(predicted)/sqrt(8))/scalingP,mean(predicted)/scalingP+(std(predicted)/sqrt(8))/scalingP,t,'r',0.8)
            ...
            t2=[-400:1:559];
            plot([t2;t2],[mnabb(300:1259)/max(mnabb(500:900))-seabb(300:1259)/(mnabb(700));mnabb(300:1259)/(mnabb(700))+seabb(300:1259)/(mnabb(700))],'g')
            plot([0 0],[-2 1.5],'k')
            plot([-192 -192],[-2 1.5],'k')
            plot([-400 300],[0 0],'k')
            xlim([-280 60])
            figure;hold on;
            plot(t,mean(actual)/mean(actual(:,280)),'k','Linewidth',3)
