function avgnote=jc_plotabunch724cbin(pitch_data,f,xmax)
...
%onset.
wn_onset=2200; %white noise onset in note - this parameter should be 
...
for i=1:length(smooth(1).smoothed)
    ...
    for j=1:length(smooth)
        if max(smooth(j).smoothed)<1*10^7
        sumnote(i)=sumnote(i)+smooth(j).smoothed(i);
        k=k+1;
        end
    end
    avgnote(i)=sumnote(i)/length(k);
end
