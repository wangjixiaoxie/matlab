function [slopesAC,slopesINA,peaksAC,peaksINA]=xcorranalyLESION(AlldataZFlesion)
% xcorr analy for UDpre/UDpost lesion datasets --- BFlesion of ZFlesion
% easily modified for UDpre/Dpre

% CHANGE g for different datasets - it's currently set for ZFs
g=[1 2 3 4 5 6];
...
for i=1:length(g)
    ...
    % modify here for UDpre/Dpre
    pitchAC1=AlldataZFlesion(gg).pitchUDpre(AlldataZFlesion(gg).basevaronset+40:AlldataZFlesion(gg).basevaroffset-40,1:23);
    pitchINA1=AlldataZFlesion(gg).pitchUDpost(AlldataZFlesion(gg).basevaronset+40:AlldataZFlesion(gg).basevaroffset-40,1:23);
