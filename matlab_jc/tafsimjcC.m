...
[avB,tb,fb]=get_avn('batchT','a',0.2,0.2,'','','obs0'); 
...
templa7=mean(avB(1:2:256,[90]),2);
size(templa6) %should be 128 point vector  (128 by 1)
%normalize template
templa7(1:6)=0;
templa7=templa7./max(templa7);
%check out template
uievtafsim('batch1006Bfiles',templa5);
...
mk_tempf('batchT',templa6,2,'obs0');
...
get_trigt2('batchtest',cntrng3,0.1,128,1,1);
...
[vals,trigs]=triglabel('batchtest','a',1,1,0,1);
...
for TH=3:-.2:2;
	cntrng6(1).TH=TH;
	get_trigt2('batchT',cntrng6,0.2,128,1,1);
	[vals,trigs]=triglabel('batchT','a',1,1,0,1);
    toff=[];
...
[vals,trigs]=triglabel('batch1007Cfiles','a',1,1,0,1);
...
wrt_templ('bk50_tmp6up.dat',templa6);
