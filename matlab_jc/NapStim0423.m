...
load /bulbul1/IntExp030612.mat
            ...
            indexps=[2:5 7:8];
            lastday=[4 3 4 4 2 3];
            count=0;clear timemins indCT indST indCTff indSTff
            for i=1:length(indexps)
                exp=indexps(i);
                % for each day
                for j=1:lastday(i)
                    day=j;
                    % pool times of stim and non-stim to find out
                    alltimes=cat(1,IntExp(exp).AllDaysCTlearning(day).tdata(:),IntExp(exp).AllDaysFBlearning(day).tdata(:));
                    lct=length(IntExp(exp).AllDaysCTlearning(day).tdata);
                    lst=length(IntExp(exp).AllDaysFBlearning(day).tdata);
                        [a,b]=sort(alltimes);
                        count=count+1;         
                        indCT{count}=find(b<lct+1);
                        indCTff{count}=IntExp(exp).AllDaysCTlearning(day).FFdata;
                        indST{count}=find(b>lct);
                        indSTff{count}=IntExp(exp).AllDaysFBlearning(day).FFdata;
                    if exp<5
                        timemins{count}=(a-min(a))/60; % time in minutes
                        indSTtime{count}=IntExp(exp).AllDaysFBlearning(day).tdata/60;
                        indCTtime{count}=IntExp(exp).AllDaysCTlearning(day).tdata/60;
                    else
                        timemins{count}=(a-min(a))*24*60; % time in minutes
                        indSTtime{count}=IntExp(exp).AllDaysFBlearning(day).tdata*24*60;
                        indCTtime{count}=IntExp(exp).AllDaysCTlearning(day).tdata*24*60;
                    end
                end
            end

            % ignore 1-4 for now b/c bk48w74 hasn't been thoroughly labeled
            clear holder holdertime
            for i=5:length(timemins)
                % for each day - gaps are greater than 5 minutes - free parameter
                chunkstarts=1+[0 find(diff(timemins{i})>0.5)'];
                for j=1:length(chunkstarts)-1
                    % stim trials in t
                    g=find(indST{i}>chunkstarts(j) & indST{i}<chunkstarts(j+1));
                    holder{i,j}=indSTff{i}(g);
                    holdertime{i,j}=indSTtime{i}(g);
                end
            end

            % Is consolidation greater between or within?
            prev=[];tprev=[];acrosscount=0;withincount=0;clear s1 vlsacross vlswithin a b tvlsacross tvlswithin
            for i=5:length(timemins)
                if i>7 & i<12
                    coef=-1;
                else
                ...
                for j=1:size(holder,2)
                    ...
                    if s1(j)>0
                        curr=holder{i,j}(1);
                        tcurr=holdertime{i,j}(1);
                        if isempty(prev)
                            prev=curr;
                            tprev=tcurr;
                        else
                            acrosscount=acrosscount+1;
                            vlsacross(acrosscount)=coef*(curr-prev);
                            tvlsacross(acrosscount)=tcurr-tprev;
                        end
                        % if one element
                        for k=2:s1(j)
                            withincount=withincount+1;
                            vlswithin(withincount)=coef*(holder{i,j}(k)-holder{i,j}(k-1));
                            tvlswithin(withincount)=holdertime{i,j}(k)-holdertime{i,j}(k-1);
                        end
                        prev=holder{i,j}(k);
                        tprev=holdertime{i,j}(k);
                    end
                end
            end
            % This is borderline significant
                % 1. There are changes between epochs of singing
                        a2=a([5 8 12 16 18]);
                        b2=b([5 8 12 16 18]);
                        figure;hold on;
                        subplot(211);hold on;
                        plot(runningmedian(vlsacross,20))
                        for i=1:length(a2)
                            plot([a2(i)-10 a2(i)-10],[-20 20],'k')
                        end
                        subplot(212);hold on;
                        plot(runningmedian(vlswithin,40))
                        for i=1:length(b2)
                            plot([b2(i)-20 b2(i)-20],[-20 20],'k')
                        end
                        mean(vlswithin(b2(1)+1:b2(2)))
                        mean(vlswithin(b2(2)+1:b2(3)))
                        mean(vlswithin(b2(3)+1:b2(4)))
                        mean(vlswithin(b2(4)+1:b2(5)))
                        mean(vlswithin(b2(5)+1:end))

                        ind1=find(tvlsacross>0 & tvlsacross<200);
                        corrcoef(tvlsacross(ind1),vlsacross(ind1)) % no correlation
                        % average gap is 16.3minutes

                        ind2=find(tvlswithin>0); % across songs, avg= -1.46, median=0.5 (n.s. from ~zero)
                        ind3=find(tvlswithin==0); % in the same song, avg = -1.16, median= -1.33 (n.s. from ~zero)
                        corrcoef(tvlswithin(ind2),vlswithin(ind2))
                        % average gap is 1.9minutes
                        ...
                        timeCT=[];
                        ffCT=[];
                        for i=5:length(indSTtime)
                            if i>7 & i<12
                                coef=-1;
                            else
                                coef=1;
                            end    
                            timeST(length(timeST)+1:length(timeST)+length(indSTtime{i}))=indSTtime{i};
                            ffST(length(ffST)+1:length(ffST)+length(indSTff{i}))=coef*indSTff{i};
                            timeCT(length(timeCT)+1:length(timeCT)+length(indCTtime{i}))=indCTtime{i};
                            ffCT(length(ffCT)+1:length(ffCT)+length(indCTff{i}))=coef*indCTff{i};
                        end

%$$$$
clear DiffST tDiffST SameST DiffST2 tDiffST2 songDiffST stimDiffST AllDiff
countSame=0;countDiff=0;countAll=0;countDiff2=0;
for i=5:length(indSTtime)
    if i>7 & i<12
        coef=-1;
    else
    ...
    % for each stimmed syllable
    for k=2:length(indSTtime{i})
        countAll=countAll+1;
        % in the same song
        if indSTtime{i}(k)==indSTtime{i}(k-1)
            countSame=countSame+1;
            SameST(countSame)=coef*(indSTff{i}(k)-indSTff{i}(k-1));
            stimDiffST(countAll)=0;
            songDiffST(countAll)=0;
            AllDiff(countAll)=SameST(countSame);
        else
            % in the previous song
            g=find(indCTtime{i}>indSTtime{i}(k-1) & indCTtime{i}<indSTtime{i}(k));
            if isempty(g)
                countDiff=countDiff+1;
                DiffST(countDiff)=coef*(indSTff{i}(k)-indSTff{i}(k-1));
                tDiffST(countDiff)=indSTtime{i}(k)-indSTtime{i}(k-1);
                stimDiffST(countAll)=tDiffST(countDiff);
                songDiffST(countAll)=tDiffST(countDiff);
                AllDiff(countAll)=DiffST(countDiff);
            else
                issong(countAll)=1;
                countDiff2=countDiff2+1;
                DiffST2(countDiff2)=coef*(indSTff{i}(k)-indSTff{i}(k-1));
                %tDiffST2(countDiff2)=indSTtime{i}(k)-indSTtime{i}(k-1);
                tDiffST2(countDiff2)=indSTtime{i}(k)-indCTtime{i}(g(end));
                stimDiffST(countAll)=indSTtime{i}(k)-indSTtime{i}(k-1);
                songDiffST(countAll)=indSTtime{i}(k)-indCTtime{i}(g(end));
                AllDiff(countAll)=DiffST2(countDiff2);
            end
        end
    end
end
    
% If singing inhibits consolidation, then we should see a reduction in
% consolidation between non-stimmed songs occurring within a minute of each
% other that is as great as the reduction in consolidation between stimmed 
% songs that occur within a minute of each other.

% % Gap since most recent song
%         ind1=find(tDiffST2>=0);
%         ffdiff1=DiffST2(ind1);
%         timegap1=tDiffST2(ind1);
%         [a,b]=sort(timegap1);
%         figure;plot(runningaverage(a,50),runningaverage(ffdiff1(b),50),'k')
%         ind=find(timegap1>0 & timegap1<1);
%         mean(ffdiff1(ind))
%         std(ffdiff1(ind))/sqrt(length(ind))
%     % -3.7 +/- 3.7 --> singing causes no appreciable reduction in consol.
%  songstimdelays=stimDiffST(find(issong));
%  c=songstimdelays(ind1);
%  d=c(ind);
%  mnstimgap=mean(d);
%  
%  mean(ffdiff1())
...
figure;hold on;
 % Gap since most recent stimmed syllable
        ind1=find(tDiffST>=0);
        ffdiff1=DiffST(ind1);
        timegap1=tDiffST(ind1);
        [a,b]=sort(timegap1);
        plot(runningaverage(a,100),runningaverage(ffdiff1(b),100))
        ind=find(timegap1>0 & timegap1<1);
        mean(ffdiff1(ind))
        std(ffdiff1(ind))/sqrt(length(ind))
    % -6.9 +/- 3.0 --> stim causes an appreciable reduction in consolidation

 % Gap since most recent stimmed syllable - songs between
    % If songs cause loss of consolidation, this curve should be lower
    tDiff_STSO=stimDiffST(find(issong));
        indX=find(tDiff_STSO>=0);
        ffdiffX=DiffST2(indX);
        timegapX=tDiff_STSO(indX);
        [a,b]=sort(timegapX);
        plot(runningaverage(a,50),runningaverage(ffdiffX(b),50),'k')
        
% Gauge significant of deviation
clear diff
    figure;hold on;
    for i=1:18
        indS=find(timegapX>i & timegapX<i+1);
        indNS=find(timegap1>i & timegap1<i+1);
        songbetween=mean(ffdiffX(indS));
        nosongbetween=mean(ffdiff1(indNS));
        %plot(i,songbetween,'ro')
        %plot(i,nosongbetween,'bo')
        diff(i)=songbetween-nosongbetween;
    end
    [h,p]=ttest(diff)
    p=0.0067 % Not the best stat to use, but definitely revealing
    plot(diff)
    plot([0 20],[0 0])
    
    % consistently less
    
% Stim causes greater reversion when there is a slight gap of about 30sec since the previous stim.   
% Therefore, stimulation causes a significant reduction in the AFP-indep
% component which takes a few seconds to appear (i.e. not within an
% individual song but across adjacent songs.
% This could be because stimulation, by scrambling the output of the AFP
% and introducing a huge signal to RA, creates temporary strengthening of 
% alternative synapses from HVC-->RA, or weakens the consolidated synapses
% by reducing the correlations, or causes LTD, or something similar.


figure;hold on;
bar(-1,mean(SameST))
plot([-1 -1],[mean(SameST)-std(SameST)/sqrt(length(SameST)) mean(SameST)+std(SameST)/sqrt(length(SameST))])
ind=find(timegap1>0 & timegap1<1);
bar(1,(mean(ffdiff1(ind))))
plot([1 1],[mean(ffdiff1(ind))-std(ffdiff1(ind))/sqrt(length(ffdiff1(ind))) mean(ffdiff1(ind))+std(ffdiff1(ind))/sqrt(length(ffdiff1(ind)))])
ind=find(timegap1>1 & timegap1<5);
bar(3,(mean(ffdiff1(ind))))
plot([3 3],[mean(ffdiff1(ind))-std(ffdiff1(ind))/sqrt(length(ffdiff1(ind))) mean(ffdiff1(ind))+std(ffdiff1(ind))/sqrt(length(ffdiff1(ind)))])
ind=find(timegap1>5 & timegap1<10);
bar(5,(mean(ffdiff1(ind))))
plot([5 5],[mean(ffdiff1(ind))-std(ffdiff1(ind))/sqrt(length(ffdiff1(ind))) mean(ffdiff1(ind))+std(ffdiff1(ind))/sqrt(length(ffdiff1(ind)))])
ind=find(timegap1>10 & timegap1<20);
bar(7,(mean(ffdiff1(ind))))
plot([7 7],[mean(ffdiff1(ind))-std(ffdiff1(ind))/sqrt(length(ffdiff1(ind))) mean(ffdiff1(ind))+std(ffdiff1(ind))/sqrt(length(ffdiff1(ind)))])
ind=find(timegap1>20);
bar(11,-3.9)
plot([11 11],[-6.8 0])


%$$$$
clear DiffST tDiffST SameST
countSame=0;countDiff=0;
for i=5:length(indSTtime)
    if i>7 & i<12
        coef=-1;
    else
...
ffdiff1=DiffST(ind1);
timegap1=tDiffST(ind1);
corrcoef(timegap1,ffdiff1)
figure;plot(timegap1,ffdiff1,'.')
[a,b]=sort(timegap1);
figure;plot(runningaverage(a,100),runningaverage(ffdiff1(b),100))
hold on;plot(runningaverage(a,100),runningaverage(ffdiff1(b),100)+runningstd(ffdiff1(b),100)/10)
hold on;plot(runningaverage(a,100),runningaverage(ffdiff1(b),100)-runningstd(ffdiff1(b),100)/10)

figure;hold on;
bar(-1,mean(SameST))
plot([-1 -1],[mean(SameST)-std(SameST)/sqrt(length(SameST)) mean(SameST)+std(SameST)/sqrt(length(SameST))])
ind=find(timegap1>0 & timegap1<5);
bar(1,(mean(ffdiff1(ind))))
plot([1 1],[mean(ffdiff1(ind))-std(ffdiff1(ind))/sqrt(length(ffdiff1(ind))) mean(ffdiff1(ind))+std(ffdiff1(ind))/sqrt(length(ffdiff1(ind)))])
ind=find(timegap1>5 & timegap1<5);
bar(3,(mean(ffdiff1(ind))))
plot([3 3],[mean(ffdiff1(ind))-std(ffdiff1(ind))/sqrt(length(ffdiff1(ind))) mean(ffdiff1(ind))+std(ffdiff1(ind))/sqrt(length(ffdiff1(ind)))])
ind=find(timegap1>5 & timegap1<10);
bar(5,(mean(ffdiff1(ind))))
...
plot([11 11],[-6.8 0])
                    ...
                    clear histo sehisto ssize
                    for i=1:50
                        histo(i)=median(ffdiff1(find(timegap1>i-1 & timegap1<i)));
                        sehisto(i)=std(ffdiff1(find(timegap1>i-1 & timegap1<i)))/sqrt(length(ffdiff1(find(timegap1>i-1 & timegap1<i))));
                        ssize(i)=length(ffdiff1(find(timegap1>i-1 & timegap1<i)));
                    end
                    figure;hold on;
                    subplot(211);hold on;
                        plot(histo);plot(histo-sehisto);plot(histo+sehisto)
                        plot([0 50],[0 0],'k')
                        xlim([0 25])
                        plot(histo,'.','Markersize',15)
                    subplot(212);hold on;
                        plot(ssize)
                        xlim([0 25])
            ...
            %%%% BEST FIGURE $$$$$$$$$$$$$$$$
                    timegap=diff(timeST);
                    ind1=find(timegap>-0.001 & timegap<100);
                    timegap1=timegap(ind1);
                    ffdiff=diff(ffST);
                    ffdiff1=ffdiff(ind1);
                    clear histo sehisto ssize
                    for i=1:50
    ...
    %%%%%%%%%%%%%%%%%% CT
                    ...
                    clear histo sehisto ssize
                    for i=1:50
                        histo(i)=mean(ffdiff1(find(timegap1>i-1 & timegap1<i)));
                        sehisto(i)=std(ffdiff1(find(timegap1>i-1 & timegap1<i)))/sqrt(length(ffdiff1(find(timegap1>i-1 & timegap1<i))));
                        ssize(i)=length(ffdiff1(find(timegap1>i-1 & timegap1<i)));
                    end
                    figure;hold on;
