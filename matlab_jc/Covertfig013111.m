...
%%%% ANALYSIS 7 - Localization within targeted note?
            ...
            for i=ind
                coef=1-2*(isequal(Experiment(i).DIR,'down')); % -1 if down, 1 if up
               changed=(coef*(mean((Experiment(i).pitchACpost(nbounds(i,1):nbounds(i,2),1:postday(i,2))'))-mean(Experiment(i).pitchACpre(nbounds(i,1):nbounds(i,2),:)')))
               plot(changed)
   ...
   %%% What about Positive Controls? Is lack of localization due to AP5 block or short-term learning? 
            ...
            figure;hold on
            for i=[1:10]
               mtargPC=median(ExperimentPC(i).TargetingWN)-32;
                coef=1-2*(isequal(ExperimentPC(i).DIR,'down')); % -1 if down, 1 if up
               changed=(coef*(mean((ExperimentPC(i).pitchWN(nboundsPC(i,1):nboundsPC(i,2),end-8:end)'))-mean(ExperimentPC(i).pitchPre(nboundsPC(i,1):nboundsPC(i,2),:)')))
               if i==2 | i==7 | i==8
                    changed=(coef*(mean((ExperimentPC(i).pitchWNcatch(nboundsPC(i,1):nboundsPC(i,2),end-8:end)'))-mean(ExperimentPC(i).pitchPre(nboundsPC(i,1):nboundsPC(i,2),:)')))
               end
               plot(changed)
               normchangedPC(i).data=changed;
            end
            centeredPC=zeros(10,1000)
            for j=[1:10]
                mnwin=round(median(ExperimentPC(j).TargetingWN-32));
                if j==9
                    mnwin=200;
                end
