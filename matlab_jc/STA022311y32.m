...
plot(windowstarts+winwidth/2-300/8,mn1s,'b')
mj=2
for i=1:size(weightedavg,1)
    mn2s(i)=mean(weightedavg(i,mj).datt(250:300));
end
plot(windowstarts+winwidth/2-250/8,mn2s,'r')
mj=3
for i=1:size(weightedavg,1)
    mn3s(i)=mean(weightedavg(i,mj).datt(640:680));
end
plot(windowstarts+winwidth/2-640/8,mn3s,'k')
mj=4
for i=1:size(weightedavg,1)
    mn4s(i)=mean(weightedavg(i,mj).datt(400:800));
end
plot(windowstarts+winwidth/2-400/8,mn4s,'g')
...
numsyllables=4;
        % A. Calculate mean pitch of each syllable irregardless of where syllable occurs in song and store in mnpd  
                % Where to look in each syllable for the harmonic stack
                        clear ptwins
                        ptwins(1).datt=300:400;
                        ptwins(2).datt=250:300;
                        ptwins(3).datt=640:680;
                        ptwins(4).datt=400:600;
        ...
        % irrespective of syllable identity.
                    ...
                    for thisfolder=1:length(neuron) % for each folder
                        for thissong=1:length(neuron(thisfolder).song) % for each song
                            ...
                            %%%%%%%
                            for thissyllable=1:length(neuron(thisfolder).song(thissong).notedata) % for each syllable
