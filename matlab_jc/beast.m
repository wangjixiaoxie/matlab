function [CSs2B,distersSELF,distersMUTUAL]=beast(CSs2,Predict)             
            for j=1:28
                    ...
                    for i=1:28
                        ...
                        NewTargs=Predict(i).onset+NormTargs-medtarg+mutualmedtarg;
                        ...
                        ii=i;
                        jj=j;
                        numpts=ptsafter+ptsbefore+1;
                                    count=0;
                                    for k=0.5:0.01:2
                                        count=count+1;
                                        %choicesMUTUAL(count)=sum(abs((k*abs(CSs2B(i,j).data(Predict(i).onset:Predict(i).onset+350))./max(abs(CSs2(i,j).data(Predict(i).onset:Predict(i).onset+350))))-Predict(j).LearnedNorm(Predict(j).onset:Predict(j).onset+350)));
                                    ...
                                    distersSELF(ii,jj)=min(choicesSELF)/numpts;
                                    distersMUTUAL(ii,jj)=min(choicesMUTUAL)/numpts;
                    end
                end