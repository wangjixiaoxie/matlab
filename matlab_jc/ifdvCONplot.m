function [ifdgram,sonogram,med]=ifdvCONplot(s,SAMPLING,N,OVERLAP,sigma,ZOOMT,ZOOMF,TL,FL)
...
g=f_e1;
g(abs(f_e1-fref)>FL)=0; g(abs(t_e1-tref)>TL)=0;
[freq,time]=size(g);
g(abs(g)<0.001)=0;
k=1;
for i=1:time
...
%[freq,time]=size(newq);
%newq(abs(newq)<0.01)=0;%Remove the points that shouldn't be there and would mess up the consensus calculation.
