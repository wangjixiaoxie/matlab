...
for i=1:size(weightedavg,1)
    mn1s(i)=mean(weightedavg(i,mj).datt(940:1040));
end
plot(windowstarts+winwidth/2-940/8,mn1s,'b')
mj=2
for i=1:size(weightedavg,1)
    mn2s(i)=mean(weightedavg(i,mj).datt(1400:1440));
end
plot(windowstarts+winwidth/2-1400/8,mn2s,'r')
mj=3
for i=1:size(weightedavg,1)
    mn3s(i)=mean(weightedavg(i,mj).datt(800:1000));
end
plot(windowstarts+winwidth/2-800/8,mn3s,'k')
% mj=4
% for i=1:size(weightedavg,1)
%     mn4s(i)=mean(weightedavg(i,mj).datt(130:180));
% end
% plot(windowstarts+winwidth/2-130/8,mn4s,'g')
...
numsyllables=3;
        % A. Calculate mean pitch of each syllable irregardless of where syllable occurs in song and store in mnpd  
                % Where to look in each syllable for the harmonic stack
                        clear ptwins
                        ptwins(1).datt=940:1040;
                        ptwins(2).datt=1400:1440;
                        ptwins(3).datt=800:1000;
        ...
        % irrespective of syllable identity.
                    ...
                    for thisfolder=1:length(neuron) % for each folder
                        for thissong=1:length(neuron(thisfolder).song) % for each song
                            ...
                            %%%%%%%
                            for thissyllable=1:3%length(neuron(thisfolder).song(thissong).noteons) % for each syllable
