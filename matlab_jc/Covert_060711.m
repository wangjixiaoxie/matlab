...
load /bulbul2/CovertAnalysis/Covert070611.mat
            ...
            plot([t1-t1 runningaverage(ExpCTL.timeWN(:),50)-min(ExpCTL.timeWN) runningaverage(ExpCTL.timePost(1:60),20)-min(ExpCTL.timeWN)-1],[p1 runningaverage(median(ExpCTL.pitchWN(w1,:))-mnpre,50) runningaverage(median(ExpCTL.pitchPost(w1,1:60))-mnpre,20)])
            plot(ExpCTL.timeWN(1:5:end)-min(ExpCTL.timeWN),median(ExpCTL.pitchWN(w1,1:5:end))-mnpre,'r.')
            plot(ExpCTL.timePost(1:2:60)-min(ExpCTL.timeWN)-1,median(ExpCTL.pitchPost(w1,1:2:60))-mnpre,'.')
            ...
            x=[0 3.5 4.5];%[t1-t1 t2-t1 t3-t1];
            y=[p1 p2 p3];e=[s1 s2 s3];
            %errorbar(x,y,e)
            ...
            plot([t1-t1 runningaverage(ExpAPV.timeAPVwn(:),15)-min(ExpAPV.timeAPVwn) runningaverage(ExpAPV.timeACpost(1:160),50)-min(ExpAPV.timeAPVwn)-12],[p1 runningaverage(median(ExpAPV.pitchAPVwn(w1,:))-mnapv,15) runningaverage(median(ExpAPV.pitchACpost(w1,1:160))-mnpre,50)])
