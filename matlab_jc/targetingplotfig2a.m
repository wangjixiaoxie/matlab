function [avgpcurvetrace,shif,sd]=targetingplotfig2a(Alldata,shiftedmethod,prctile_cutoff,tmp_cutoff,notesizeshiftms)
...
for i=1:length(Alldata)
    ...
    for j=1:length(exp)
        if exp(1).acsf(j)==1 && exp(1).baseline(j)==0
            ...
            y(count)=mean(mean(exp(j).selectedpitchcurves(880-40:880+40,:)'));
            z(count).toffs=exp(j).toffset;
            z(count).curves=exp(j).selectedpitchcurves;
        end
    end
    % 40 points is 5ms
    avgbaseline=mean(Alldata(i).baselineAC(880-40:880-40,:)');
