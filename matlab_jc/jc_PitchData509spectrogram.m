function [pitch_data,ifd,sono]=jc_PitchData509(N,OVERLAP,sigma,F_low,F_high,batchFWN,noteFWN,prenoteFWN,postnoteFWN)

% Convolution of autocorrelation function with a gaussian smoothing window
...
for found_note=1:length(fvalsstr)
    ...
    [ifdgram,sonogram]=ifdv(current_note,SAMPLING,128,124,sigma,ZOOMT,ZOOMF,TL,FL); 
    ...
    for current_bin=1:total_bins
        this_slice=sonogram(:,current_bin);
        c=xcov(this_slice);
        %It looks smooth enough already without the convolution
