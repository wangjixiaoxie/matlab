function [nn,notes]=jc_EXPcontingency(pitch,chspec,BarHeight,BarDirection,chunk)
...
for i=1:size(norm,2)
    ...
    mlater(i)=mean(norm(chunk(1)+200:chunk(2)+200,i));
end
ss=std(mm);
...
if strcmp(BarDirection,'above')
    ...
    %%%Above contingency
    for i=1:size(norm,2)
        if mm(i)>(mean(mm)+BarHeight*ss) && mlater(i)<mean(mlater)
            nn(:,count)=norm(:,i);
            notes(i)=10;
            count=count+1;
        end
