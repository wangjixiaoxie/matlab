...
% occur and var reduction to occur
    ...
    shiftingMU=[];
      ...
      figure;subplot(121);hold on; 
        yvalsAC=(runningaverage([mean(shiftingAC(240:280,:)) ],20));
        xvalsAC=(runningaverage([mean(shiftingAC(220:410,:)) ],20));
        plot(xvalsAC,yvalsAC,'*')    
        yvalsMU=(runningaverage([mean(shiftingMU(240:280,:)) ],20));
        xvalsMU=(runningaverage([mean(shiftingMU(220:410,:))],20));
        plot(xvalsMU,yvalsMU,'*','Color','r')        
        subplot(122);hold on;
        mnbaseACy=mean(mean(AD10.baselineAC(240:280,:)));
        mnbaseACx=mean(mean(AD10.baselineAC(220:410,:)));
        mnbaseMUy=mean(mean(AD10.baselineINA(240:280,:)));
        mnbaseMUx=mean(mean(AD10.baselineINA(220:410,:)));
        plot((xvalsAC-mnbaseACx),(yvalsAC-mnbaseACy),'*')    
        plot((xvalsMU-mnbaseMUx),(yvalsMU-mnbaseMUy),'*','Color','r')        
        plot([0 150],[0 150],'k')               
% pk20r49
