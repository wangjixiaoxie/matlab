...
%Analysis of FF at one point in time
...
%DETERMINE WHETHER stim trials were catch or stim.
...
run_num=avls.catchstimfv(ii);
   notind{run_num}=[]
    ...
    %these values are useful for plotting histograms
...
%loop through each batch file specified in analfv
%and get time of stim
for ii=1:length(avls.analfv)
    ...
    %if avls.REMOVEOUTLIERS==1,
    %this code removes values greater than avls.STD away from mean
    if(isfield(avls,'REMOVEOUTLIERS'))
       if(avls.REMOVEOUTLIERS)
           ...
           avls.fbmeanlim(run_num)=mean(vals(crfbindlim,2));
