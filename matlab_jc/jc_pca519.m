...
%Normalize data to remove the first principle component
for i=1:length(forpca)
    m=mean(forpca(i).pitches);
    for j=1:length(forpca(i).pitches)
        forpca(i).pitches(j)=forpca(i).pitches(j)-m;
    end
end
