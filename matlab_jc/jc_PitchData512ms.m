function [fvalsstr,pitch_data,ifd,sono]=jc_PitchData512ms(N,OVERLAP,sigma,F_low,F_high,batchFWN,noteFWN,prenoteFWN,postnoteFWN)
...
for found_note=1:length(fvalsstr)
    ...
    %current_note=bandpass(current_note,SAMPLING,500,15000,ChanSpec);
    chopped_note=current_note(400:2200);
    [ifdgram,sonogram]=ifdv(chopped_note,SAMPLING,N,OVERLAP,sigma,ZOOMT,ZOOMF,TL,FL); 
    ...
    for currenttime_bin=1:total_bins
        ...
        for i=1:highest_harmonic
            ...
            Powerful(i)=Pow;
            Freqbinest(i)=mini-((Ind-0.5)/i); %e.g. (250-((1-0.5)/i)=249.5 for harmonic 1, 249.75 for harmonic 2, etc...
