function [avgpcurvetrace]=targetingplotfig2short(Alldata,shiftedmethod,prctile_cutoff,tmp_cutoff,notesizeshiftms)
...
for aaa=1:length(Alldata(1).ind_shortnotes);
    n=Alldata(1).ind_shortnotes(aaa);
left=bshift(n)-1;
...
plot(xaxis,avcurve,'k');hold on;plot(xaxis,avup,'k');hold on;plot(xaxis,avdn,'k')
xlim([0 85])
