    ...
    if Exp(i).learning % For learning experiments
        ...
        for j=1:length(Exp(i).time) % Pre, Dur, Post
            if Exp(i).JC==0 % Time data for Evren vs. JC
                ...
                day1=floor(min(Exp(i).time{1}));
                for ii=1:length(unistart) % For each day - +0.1 ensures that you don't have any boundary conditions (e.g. 24.5hrs and 47.8 hrs)
                    daycount=floor(min(Exp(i).time{j}(unistart(ii):uniend(ii))))-day1+1;
                ...
                unistart=[1 unidays+1];
                uniend=[unidays length(Exp(i).time{j})];
                day1=floor(min(Exp(i).time{1})/24);
                for ii=1:length(unistart) % For each day - +0.1 ensures that you don't have any boundary conditions (e.g. 24.5hrs and 47.8 hrs)
                    daycount=floor(min(Exp(i).time{j}(unistart(ii):uniend(ii)))/24)-day1+1;
                    allrpdata{daycount}=Exp(i).rplength{j}(unistart(ii):uniend(ii));
                    group(daycount)=j;
                end
            end
