function [neuron]=HVCprocessing_b75w54(folders,clusters,prefixes,isDirected)
% neuron(folder)
%	 .song(song)
...
for mm=mmat %each folder
    clearvars -except  mm folders neuron clusters prefixes allvals isDirected
    m=mm;
    ...
    for ifn=1:length(ff) % for each song
        ...
        allnts='aabc';%unique(labels);
        windowingFF(1,:)=[1300 1500];windowingFF(2,:)=[1300 1500];windowingFF(3,:)=[1300 1500];windowingFF(4,:)=[2600 3000];
        for i=1:length(allnts) % for each note present in this song
            ...
            for ii=1:length(p) % for each instance of that note in this song
              ...
              % calculate pitch     
                    ...
                    harms=1;%floor(Nyquist/windowingFF(ii,2));
...
% clear syllable
% for i=1:length(allnts) % for each syllable
%     clear allspec
%     count=0;
%     for j=1:5 % for each song
%         for k=1:length(allsongs(j).syllable(i).rendition)
%             count=count+1;
%             allspec(count,:,:)=allsongs(j).syllable(i).rendition(k).data;
%         end
%     end
%     ka=mean(allspec);
%     kb=reshape(ka,[513 1745]);
%     syllmn(i).data=kb;
% end
% 
% tvals=[1/8:1/8:1745/8];
% ffvals=[Nyquist:-Nyquist/513:Nyquist/513];
% figure;hold on;
% for i=1:length(allnts)
%     subplot(1,length(allnts),i)
