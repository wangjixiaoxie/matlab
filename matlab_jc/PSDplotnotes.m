...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  When using D vs. UD instead of prelesion vs. postlesion, BFs look much
%  more like ZFs, with a peak around 16ms.  But there are only 2 BF
%  Directed song samples where the CV is reduced.
...
% In undirected song, LMAN-->RA projection neurons fire bursts at variable
% times.  One hypothesis is that LMAN bursts can disrupt the natural, highly
% stereotyped pattern of RA bursts, producing extra RA bursts (via direct
% excitation driving an RA neuron above threshold) or eliminating typically
% present RA bursts (indirectly via interneurons).  Given the previous
% results that ZF RA bursts last ~ 10ms and control ~10ms chunks of song,
% LMAN bursting should produce variation in ZF song parameters that last
% ~10ms.  We are particularly concerned with the song parameter of pitch,
% since variation in this parameter has been linked to "pitch shift
% learning", the phenomenon that this study is motivated by.  To precisely
% measure pitch dynamics in different LMAN conditions, we use the Hilbert
% transform 
