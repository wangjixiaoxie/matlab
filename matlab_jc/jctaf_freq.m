function vals=jctaf_freq(bt,fbins,NT,NFFT,CS,USEX,USEX_TRIG);
...
for ii=1:length(ff)
	...
	for jj=1:length(tt)
		...
		if (length(pp2)>0)
            nhits=nhits+1;
			TRIG=1;
            if rdtemp.catch(nhits)==0 %If not catch note
                WNblast=1; %wn blast
            else
                WNblast=0; %catch note
            end
		else
			TRIG=0;
            WNblast=0;
		...
		vals=[vals;fn2datenum(fn),tempvals,IND,filenum,TRIG,WNblast];
	end
end
return;
