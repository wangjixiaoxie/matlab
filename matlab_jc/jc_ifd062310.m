function [IFDS]=jc_ifd062310(shifted,N,OVERLAP,sigma,F_low,F_high,harms,filetype,returnAV)
...
for found_note=1:NumberOfNotes
    ...
    sonogram=ifdv(shifted(found_note,:),SAMPLING,N,OVERLAP,sigma,1,1,3,3);
    %(flipdim(spectrogram(shifted(found_note,:),w,OVERLAP,N,SAMPLING),1)); %gaussian windowed spectrogram
    IFDS(found_note).data=sonogram;
    ...
    for currenttime_bin=1:size(sonogram,2)
        ...
        for i=1:length(harms)
            ...
            if Ind==1 || Ind==length(freq_window)
                ...
                Indest=Ind; %pinterp([Ind-1;Ind;Ind+1], [freq_window(Ind-1);freq_window(Ind);freq_window(Ind+1)]);
            end
