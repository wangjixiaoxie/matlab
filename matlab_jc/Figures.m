...
% Figure 4 - lines ~2300 in 416.m
            ...
            plot(2,BestSelfCSnorm64,'.','MarkerSize',20,'Color','r')
                ...
                %%%%%%
figure;hold on;
plot(BestSelfCSnorm64,BestOtherCSnorm64,'.','MarkerSize',20,'Color','b')
plot([0 0.17],[0 0.17],'k')          
xlim([0 0.17]);ylim([0 0.17])
plot([mean(BestSelfCSnorm64)-std(BestSelfCSnorm64)/sqrt(28) mean(BestSelfCSnorm64)+std(BestSelfCSnorm64)/sqrt(28)], [mean(BestOtherCSnorm64) mean(BestOtherCSnorm64)],'k')
plot( [mean(BestSelfCSnorm64) mean(BestSelfCSnorm64)],[mean(BestOtherCSnorm64)-std(BestOtherCSnorm64)/sqrt(28) mean(BestOtherCSnorm64)+std(BestOtherCSnorm64)/sqrt(28)],'k')                
...
% Figure 6B - pred/actual/heat maps comparing shifts
    % Heat maps - see 416.m
    figure;hold on;
    subplot(131);hold on;
    plot(CSs64(4,4).data/max(abs(CSs64(4,4).data(Predict(4).onset:Predict(4).offset))),'r')
    plot(Predict(4).LearnedNorm,'k')
    xlim([Predict(4).onset+7 Predict(4).offset])
    ylim([-1.1 1.1])
    plot([Predict(4).onset+7 Predict(4).offset],[0 0])
    [xS,yS]=hist(Predict(4).Targeting-32,20);
    xS(1)=0;
    stairs(yS,1*(xS/length(Predict(4).Targeting)),'LineWidth',2,'Color','g')

    subplot(132);hold on;
    plot(CSpred(1).data/max(abs(CSpred(1).data(DShifts(1).onset:DShifts(1).offset))),'r')
    plot(shifted(1).data/max(abs(shifted(1).data(DShifts(1).onset:DShifts(1).offset))),'k')
    xlim([DShifts(1).onset DShifts(1).offset])
    ylim([-1.1 1.1])
    [xS,yS]=hist(DShifts(1).toffset-32,20);
    stairs(yS,1*(xS/length(DShifts(1).toffset)),'LineWidth',2,'Color','g')
    stairs(yS+192,1*(xS/length(DShifts(1).toffset)),'LineWidth',2,'Color','g')
    plot([DShifts(1).onset DShifts(1).offset],[0 0])
    
    subplot(133);hold on;
    xlim([DShifts(2).onset DShifts(2).offset])
    plot(CSpred(2).data/max(abs(CSpred(2).data(DShifts(2).onset:DShifts(2).offset))),'r')
    plot(shifted(2).data/max(abs(shifted(2).data(DShifts(2).onset:DShifts(2).offset))),'k')
    ylim([-1.1 1.1])
    [xS,yS]=hist(DShifts(2).toffset-32,40);
    xS(1)=0;
    stairs(yS,1*(xS/length(DShifts(2).toffset)),'LineWidth',2,'Color','g')
    stairs(yS+192,1*(xS/length(DShifts(2).toffset)),'LineWidth',2,'Color','g')
    plot([DShifts(2).onset+7 DShifts(2).offset],[0 0])
% Figure 6C - mean +/-SE
                figure;hold on;
                     actual=zeros(8,2000);
...
%                 end
                ...
                j2=max(mnabbT(158:1000));
                %j3=max(mnabbNT(158:1259));
                plot(t/8,mnabbT(158:1259)/j2,'g','LineWidth',3) % targ imprecision included
...
% Figure 7A
        ...
        for i=1:size(mnccAC,1)
            ...
            stdAC2(i)=std(mnccAC(i,ind1).^2)/sqrt(length(ind1));
            ind2=find(mnccINA(i,:)>0);
            mnccINA2(i)=mean(mnccINA(i,ind1).^2);
            stdINA2(i)=std(mnccINA(i,ind1).^2)/sqrt(length(ind2));
        end
        %% THIS PLOTS IT
        t1=[1:1:length(mnccAC2)];
        figure;hold on;
        plot([t1;t1],[(mnccAC2-stdAC2);(mnccAC2+stdAC2)],'-','Color','r')
        plot([t1;t1],[(mnccINA2-stdINA2);(mnccINA2+stdINA2)],'-','Color','b')
