...
figure;plot(x);hold on;plot(y,'r')
  ...
  %%%%%%%%%%
            for i=[1:6]
                i
                data=AllEscs(i).data';
                % Filter out everything faster than 40*sampling rate (i.e. 5ms)
                ...
                distH(i).data=(abs(Actual(i).data/max(Actual(i).data)-HitPred(i).data/max(HitPred(i).data)));
                 ...
                 distES(i).data=(abs(Actual(i).data/max(Actual(i).data)-abs(y/max(y))));
                 dHm(i)=mean(distH(i).data);
                 dEm(i)=mean(distE(i).data);
                 dES(i)=mean(distES(i).data);
            end
            
           figure;hold on;
        for k=1:6
        subplot(3,2,k);hold on;
        for i=1:length(predJ(k).data)
            plot([i i],[predJ(k).data(i)/max(predJ(k).data) Actual(k).data(i)/max(Actual(k).data)])
        end
        plot(predJ(k).data/max(predJ(k).data),'b','Linewidth',2)
        plot(Actual(k).data/max(Actual(k).data),'k','Linewidth',2)
        ylim([0 1.05])
        xlim([0 length(Actual(k).data)])
        end
...
ptwindfilt=[min(ptwindow)-200:max(ptwindow)+200];
        for i=1:length(indE1)
            AllEscs(1).data(i,:)=(Experiment(1).pitchWNon(ptwindfilt,indE1(i))')-mean(Experiment(1).pitchBaseline(ptwindfilt,1:60)');
        end
        for i=1:length(indH1)-1
            AllHits(1).data(i,:)=(Experiment(1).pitchWNon(ptwindfilt,indH1(i+1))')-mean(Experiment(1).pitchBaseline(ptwindfilt,1:60)');
        end
...
ptwindfilt=[min(ptwindow)-200:max(ptwindow)+200];
        for i=1:length(indE1)
            AllEscs(2).data(i,:)=(Experiment(2).pitchWNon(ptwindfilt,indE1(i))')-mean(Experiment(2).pitchBaseline(ptwindfilt,:)');
        end
        for i=1:length(indH1)
            AllHits(2).data(i,:)=(Experiment(2).pitchWNon(ptwindfilt,indH1(i))')-mean(Experiment(2).pitchBaseline(ptwindfilt,:)');
        end
...
ptwindfilt=[min(ptwindow)-200:max(ptwindow)+200];
        for i=1:length(indE1)
            AllEscs(3).data(i,:)=(Experiment(3).pitchWNon(ptwindfilt,indE1(i))')-mean(Experiment(3).pitchBaseline(ptwindfilt,[1:4 6:35 37:end])');
        end
        indHx=indH1([1:5 7:end]);
        for i=1:length(indHx)
            AllHits(3).data(i,:)=(Experiment(3).pitchWNon(ptwindfilt,indHx(i))')-mean(Experiment(3).pitchBaseline(ptwindfilt,[1:4 6:35 37:end])');
        end
...
%%%% Experiment # 4 - r87g80
 ...
 ptwindfilt=[min(ptwindow)-200:max(ptwindow)+200];
        for i=1:length(indE1)
            AllEscs(4).data(i,:)=(Experiment(4).pitchWNon(ptwindfilt,indE1(i))')-mean(Experiment(4).pitchBaseline(ptwindfilt,:)');
        end
        indHx=indH1([1:end]);
        for i=1:length(indHx)
            AllHits(4).data(i,:)=(Experiment(4).pitchWNon(ptwindfilt,indHx(i))')-mean(Experiment(4).pitchBaseline(ptwindfilt,:)');
        end
...
%%%% Experiment # 5 - bk80w28
 ...
 ptwindfilt=[min(ptwindow)-200:max(ptwindow)+200];
        for i=1:length(indE1)
            AllEscs(5).data(i,:)=(Experiment(5).pitchWNon(ptwindfilt,indE1(i))')-mean(Experiment(5).pitchBaseline(ptwindfilt,[1:4 6:35 37:end])');
        end
        indHx=indH1([1:end]);
        for i=1:length(indHx)
            AllHits(5).data(i,:)=(Experiment(5).pitchWNon(ptwindfilt,indHx(i))')-mean(Experiment(5).pitchBaseline(ptwindfilt,[1:4 6:35 37:end])');
        end
...
% Experiment #6 - bk91w60
 ...
 ptwindfilt=[min(ptwindow)-200:max(ptwindow)+200];
        for i=1:length(indE1)
            AllEscs(6).data(i,:)=(Experiment(6).pitchWNon(ptwindfilt,indE1(i))')-mean(Experiment(6).pitchBaseline(ptwindfilt,[1:38 41:end])');
        end
        indHx=indH1([1:end]);
        for i=1:length(indHx)
            AllHits(6).data(i,:)=(Experiment(6).pitchWNon(ptwindfilt,indHx(i))')-mean(Experiment(6).pitchBaseline(ptwindfilt,[1:38 41:end])');
        end
