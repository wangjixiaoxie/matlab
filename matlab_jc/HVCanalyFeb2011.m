    ...
    realtime=windowingT(i,:)-16;
        for k=1:size(fvsub(i).sonsv,1) % for each rendition
            ...
            for kk=1:length(fvsub(1).struc(1).chan) % for each cluster
                ...
                numspikes(i).data(k).chan(kk)=length(find(fvsub(i).struc(k).chan(kk).spiketms*1e3>mintm & fvsub(i).struc(k).chan(kk).spiketms*1e3<maxtm));
            end
        end
end
...
% Then narrow this down by which ones are actually neurons...