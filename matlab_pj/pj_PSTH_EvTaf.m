function pj_PSTH_EvTaf(fn,datChan,auxChan,fs,stimLength,trialLength,binSizes,xNoise,saveFigs,plotRawDat,plotPSTH,plotRaster)
%% Make a PSTH and raster with user input data. For neural data recorded with EvTaf (cbin files). 
...
if saveFigs == 1
    h = gcf;
    t = [shortFn,'_rawData_detectedSpikes_channel',num2str(datChan),'_',num2str(spikeThresh),'spikeThreshold_withLaser.fig'];
    saveas(h, t)
end
...
if plotPSTH == 1
    for b = 1:length(binSizes)
        ...
        xlabel('Time (s)')
        ylabel('Spikes / Bin')
        title(sprintf(['PSTH for file ',shortFn,', spike threshold = ',num2str(spikeThresh),'\n'...
...
if plotRaster == 1
    ...
    title(sprintf(['Raster plot for file ',shortFn,', spike threshold = ',num2str(spikeThresh),'\n'...
        'stim length = ',num2str(sL),'ms, trial period = ',num2str(tL),'ms\n',...
        num2str(numTrials),' trials, laser head power = 30mW\n Right RA: 2.7mm Ventral, Channel ',num2str(datChan)]),...
        'FontWeight','bold')
