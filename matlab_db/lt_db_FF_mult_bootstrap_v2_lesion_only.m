% 11/15/13 - added "lesion_only," in that for some analyses there is no
% option for lesion or WN, it is just using lesion parameters. (i.e. post
% line 604)

% 11/05/13 - modified by LT to go with
% lt_db_contour_and_FF_analysis_over_time_v4.m, where I have option of
% analyzing either lesion or WN data - the choice is saved as a variable
% in the multiple_pitch structure.  (important for plotting only (e.g.
% baseline, lines for lesion, etc.)
...
% Different if WN or lesion experiment
if multiple_pitch.parameters.WN_question=='y';
    ...
    % the following if was lesion experiment.
elseif multiple_pitch.parameters.lesion_question=='y';
    for i = 1:length(multiple_pitch.parameters.syllables)
        temp_baseline =...
            zeros([mult_FF_boot.parameters.numbootstrap length(mult_FF_boot.parameters.names_parameters) multiple_pitch.parameters.lesion_begin]);
    ...
    for j = 1:length(mult_FF_boot.parameters.names_parameters)
        ...
        elseif multiple_pitch.parameters.lesion_question=='y';
            xlim([-1-multiple_pitch.parameters.lesion_begin...
                multiple_pitch.parameters.days{4}-multiple_pitch.parameters.days{3}-multiple_pitch.parameters.lesion_begin+1])
        end
        ...
        elseif multiple_pitch.parameters.lesion_question=='y';
            for k = 1:length(mult_FF_boot.(multiple_pitch.parameters.syllables(i)).bootstrap)
                try
                    %plots median from bootstrap
                    plot(k-1-multiple_pitch.parameters.lesion_begin,...
                        mult_FF_boot.(multiple_pitch.parameters.syllables(i)).median{k}(j),'o',...
                        'MarkerSize',10,...
                        'MarkerFaceColor', [0.1 0.1 0.1],...
                        'MarkerEdgeColor', [0.2 0.8 0.3])
                    
                    %plots 95% CI from bootstrap
                    line([k-1-multiple_pitch.parameters.lesion_begin k-1-multiple_pitch.parameters.lesion_begin],...
                        [mult_FF_boot.(multiple_pitch.parameters.syllables(i)).CI{k}(1,j)...
                        mult_FF_boot.(multiple_pitch.parameters.syllables(i)).CI{k}(2,j)],...
                        'LineWidth', 2,...
                        'Color', [0.2 0.2 0.7])
            ...
            for ii=1:length(line_index);
                try
                    line([line_index(ii)-0.25...
                        line_index(ii)-0.25],...
                        get(gca, 'YLim'),...
                        'Color', [0 0 0],...
                        'LineStyle', '--')
...
for i = 1:length(multiple_pitch.parameters.syllables)
    ...
    xlabel('day')
    xlim([-1-multiple_pitch.parameters.lesion_begin... % MODIFIED
        multiple_pitch.parameters.days{4}-multiple_pitch.parameters.days{3}-multiple_pitch.parameters.lesion_begin+1])
