%% To check sequence template matching, hit rate, and trigger timing
...
mkdir(['check_sequence_timing_' today_date])
...
if strcmpi(load_previous_parameters, 'y')
    num_syl = input('how many transition points?  ');
    tpofinterest = cell(1,num_syl);
    for i = 1:num_syl
        tpofinterest{i} = input(['what is the name of transition point #' num2str(i) '?  '], 's');
...
elseif strcmpi(load_previous_parameters, 'n')
    ...
    for i = 1:numberoftranisitionpoints
        tpofinterest{i} = input(['what is transition #' num2str(i) '?  '], 's');
        ...
        for m = 1:syllables.(tpofinterest{i}).parameters.num_diff_cntrng
            if m == 1
...
%     end
    ...
    saveas(figure(i), ['check_sequence_timing_' today_date '/Toff_distro_' tpofinterest{i} '_(triggered)_' date_of_exp], 'fig')
...
%     end
    ...
    xlabel('Syllable number')
    ylabel('Timing offset (ms)')
    saveas(figure(10*i), ['check_sequence_timing_' today_date '/Toff_distro_' tpofinterest{i} '_(all)_' date_of_exp], 'fig')
...
for i = 1:length(tpofinterest)
    display(' ')
    display(['branch point: ' tpofinterest{i} ' hit rate information'])
...
for i = 1:length(tpofinterest)
    display(' ')
    display(['branch point: ' tpofinterest{i} ' match rate information'])
...
save(['check_sequence_timing_' today_date '/check_sequence_timing_' today_date])

%move batch files to new folder, make sure the batch file name is not too
%similar to the name of your bird
if strcmpi(input(['Move batch files to check_sequence_timing_' today_date ' folder? (y or n)  '], 's'),'y')
    movefile([batchfile(1:4) '*'],['check_sequence_timing_' today_date])
else
end
